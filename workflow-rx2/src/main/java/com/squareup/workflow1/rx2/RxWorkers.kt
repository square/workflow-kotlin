 ...
 * [Observable] will first be adapted to a flow. To do this a communication structure is used that
 * will mean that the flow will process the items produced by the [Observable] asynchronously.
 *
 * Note that this [Observable] is transformed to a backpressure aware [Flowable] using the [BUFFER]
 * strategy before being transformed to a flow, which means that buffering will happen in the
 * [Flowable] before being passed over to the flow if it is experiencing backpressure.
 *
 * RxJava doesn't allow nulls, but it can't express that in its types. The receiver type parameter
 * is nullable so that the resulting [Worker] is non-nullable instead of having platform
 * nullability.
 */
 ...
 *
 * Note that a [Worker] actually operates on a [kotlinx.coroutines.flow]. This means that your
 * [Publisher] will first be mapped to a flow. To do this a communication structure is used that
 * will mean that the flow will process the items produced by the [Publisher] asynchronously.
 *
 * RxJava doesn't allow nulls, but it can't express that in its types. The receiver type parameter
 * is nullable so that the resulting [Worker] is non-nullable instead of having platform
 * nullability.
...
public inline fun <reified T : Any> Publisher<out T?>.asWorker(): Worker<T> =
  ...
  @Suppress("UNCHECKED_CAST") (this as Publisher<T>).asFlow().asWorker()
 ...
 * nullability.
 */
public inline fun <reified T : Any> Maybe<out T?>.asWorker(): Worker<T> = Worker.fromNullable {
  awaitSingleOrNull()
}
