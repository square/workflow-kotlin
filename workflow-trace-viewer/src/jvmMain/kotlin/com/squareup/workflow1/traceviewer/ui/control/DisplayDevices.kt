...
@Composable
internal fun DisplayDevices(onDeviceSelect: (String) -> Unit, modifier: Modifier = Modifier) {
  val devices by
    produceState(initialValue = listDevices()) {
      while (true) {
    ...
    Column {
      devices.forEach { device ->
        key(device) {
          Card(
            onClick = {
              emulatorRegex.find(device)?.value?.let { emulator -> onDeviceSelect(emulator) }
            },
...
private fun listDevices(): List<String> {
  ...
  val devices = process.inputStream.use { it.bufferedReader().readLines().drop(1).dropLast(1) }
  ...
  return devices.mapNotNull { device ->
    ...
    deviceName.waitFor()
    "$deviceId " +
      deviceName.inputStream.use { it.bufferedReader().readLines().firstOrNull() ?: "" }
  }
}
