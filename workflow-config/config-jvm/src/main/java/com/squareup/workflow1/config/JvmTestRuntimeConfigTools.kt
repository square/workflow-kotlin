...
public class JvmTestRuntimeConfigTools {
  public companion object {
    /**
     * Helper for Configuration for the workflow runtime in an application. This allows one to
     * specify a project property from the gradle build to choose a runtime. e.g. add
     * `-Pworkflow.runtime=conflate` in your gradle build to build the conflate runtime into the
     * application.
     *
     * The [WorkflowTestRuntime][com.squareup.workflow1.testing.WorkflowTestRuntime] and
     * [RenderTester][com.squareup.workflow1.testing.RenderTester] runtimes already call this
     * utility. To honor this property from your own runtime call this function and pass the result
     * to the call to [renderWorkflowIn][com.squareup.workflow1.renderWorkflowIn] as the
     * [RuntimeConfig] parameter.
     *
     ...
     *   action cascade.
     *
     * - `partial` Partial tree rendering, which only re-renders each Workflow node if: 1) its state
     *   changed; or 2) one of its descendant's state changed. (This option requires `stateChange`,
     *   and enables it as well.)
     *
     ...
     *   that are queued on nodes not affected by the last action application, then we will continue
     *   to process those actions before another render pass.
     *
    ...
    public fun getTestRuntimeConfig(): RuntimeConfig {
      val selection =
        System.getProperty("workflow.runtime", "")
          .split("-")
