  ...
  public fun <T> runRethrowingUncaught(block: () -> T): T {
    val result =
      try {
        ...
        // If any uncaught exceptions were reported during block's execution, mark them as
        // suppressed.
 ...
 * [default uncaught exception handler][Thread.defaultUncaughtExceptionHandler] while running
 * [block], and then after `block` returns, rethrows any exceptions that were reported to the
 * uncaught handler.
 *
...
public fun rethrowingUncaughtExceptions(block: () -> Unit) {
  ...
  Thread.setDefaultUncaughtExceptionHandler { _, e -> guard.reportUncaught(e) }
  try {
