...
/**
 * Create a [RenderTester] to unit test an individual render pass of this [SessionWorkflow], using
 * the workflow's [initial state][StatefulWorkflow.initialState], in the [workflowScope].
 *
 * See [RenderTester] for usage documentation.
 */
...
/**
 * The props must be specified, the initial state may be specified, and then all child workflows and
 * workers that are expected to run, and any outputs from them, must be specified with
 * [expectWorkflow] and (optionally) [expectWorker] and [expectSideEffect] calls. If one needs to
 * verify all workers explicitly, perhaps to verify that a worker is *not* run, then use
 * [requireExplicitWorkerExpectations]. Likewise [requireExplicitSideEffectExpectations] for side
 * effects. Then call [render] and perform any assertions on the rendering. An event may also be
 * sent to the rendering if no workflows or workers emitted an output. Lastly, the
 * [RenderTestResult] returned by `render` may be used to assert on the [WorkflowAction]s processed
 * to handle events or outputs by calling [verifyAction][RenderTestResult.verifyAction] or
 ...
 *   pass do not cause a test failure unless [requireExplicitWorkerExpectations] is true. Side
 *   effects are optionally specified. Specified side effects must run. Unexpected side effects on a
 *   render pass do not cause a test failure unless [requireExplicitSideEffectExpectations] is true.
 ...
 * ## Testing Application
 *
 * Note that by providing the values for all child workflows, workers, and side effects, the test is
 * extremely focused on the logic of the `render()` method. Everything else is essentially 'mocked'.
 * If you do not have complex logic within `render()` that needs to be directly unit tested, then
 * you may be better served by using a [WorkflowTurbine] and [renderForResult] as this will actually
 * run your workflow (and any children, fake or not) in the runtime.
 *
 * ## Examples
 *
 * ### Worker output
 *
 * The following example tests a render pass that runs one worker, `SubmitLoginWorker`, which is
 * configured to have "emitted" an output, and one workflow, `ChildWorkflow`, which expects a props
 * containing "test@foo.com" and returning a `ChildRendering` as its rendering.
 *
 ...
 *
 * This test verifies the action _result_ instead of the action itself. This technique is useful if
 * the [WorkflowAction] is anonymous or inline.
 *
 *    ```
 *    val currentState = …
 *    val previousState = …
...
public abstract class RenderTester<PropsT, StateT, OutputT, RenderingT> {
   ...
   *
   * @param description String that will be used to describe this expectation in error messages. The
   *   description is required since no human-readable description can be derived from the predicate
   *   alone.
   * @param exactMatch If true, then the test will fail if any other matching expectations are also
   *   exact matches, and the expectation will only be allowed to match a single child workflow. If
   *   false, the match will only be used if no other expectations return exclusive matches (in
   ...
   *   alone.
   * @param exactMatch If true, then the test will fail if any other matching expectations are also
   *   exact matches, and the expectation will only be allowed to match a single side effect. If
   *   false, the match will only be used if no other expectations return exclusive matches (in
   *   which case the first match will be used), and the expectation may match multiple side
   *   effects.
  ...
  /**
   * Specifies that this render pass is expected to remember a calculated value with parameters that
   * satisfy [matcher]. This expectation is strict, and will fail if multiple side effects match.
   *
   * @param description String that will be used to describe this expectation in error messages. The
   *   description is required since no human-readable description can be derived from the predicate
   *   alone.
   ...
   *   [RenderContext.remember][com.squareup.workflow1.BaseRenderContext.remember] and return true
   *   if such a call expected.
   */
   ...
   *
   * All workflows rendered/ran by the workflow must be specified before calling this method.
   * Workers are optionally specified.
   *
   * @param block Passed the result of the render pass to perform assertions on. If no child
   *   workflow or worker was configured to emit an output, may also invoke one of the rendering's
   *   event handlers. It is an error to invoke an event handler if a child emitted an output.
 ...
 * rendered, it will match any expectation that specifies the type of that workflow, or any of its
 * supertypes. This means that if you have a workflow that is split into an interface and a concrete
 * class, your render tests can pass the class of the interface to this method instead of the actual
 * class that implements it.
 *
 * Note that Workflow<Int, String, Int> is *not* a sub-type of Workflow<Int, Object, Int> because it
 * is not covariant for the [OutputT] generic (the same is true for [PropsT]). This means that you
 * cannot use the [WorkflowIdentifier] or [KClass] of a Workflow class whose [OutputT] or [PropsT]
 * are supertypes to the one you want to match. If this is the only reasonable class definition you
 * have access to, then consider using [expectCovariantWorkflow] and specifying those types
 * explicitly.
 *
...
public inline fun <ChildRenderingT, PropsT, StateT, OutputT, RenderingT> RenderTester<
  PropsT,
  StateT,
  OutputT,
  RenderingT,
 ...
 *   messages.
 */
public fun <ChildOutputT, ChildRenderingT, PropsT, StateT, OutputT, RenderingT> RenderTester<
  PropsT,
  StateT,
  OutputT,
  RenderingT,
>
  ...
  expectWorkflow(
    ...
    if (
      invocation.workflow.identifier.realTypeMatchesClassExpectation(identifier) &&
 ...
 *    ```
 *
 * @param workflowType The [KClass] of the expected workflow. May also be any of the supertypes of
 *   the expected workflow, e.g. if the workflow type is an interface and the workflow-under-test
 *   injects a fake. See note above about covariance with [PropsT] and [OutputT] and how these
 *   cannot help with supertypes.
 * @param rendering The rendering to return from
 *   [renderChild][com.squareup.workflow1.BaseRenderContext.renderChild] when this workflow is
 *   rendered.
 * @param key The key passed to [renderChild][com.squareup.workflow1.BaseRenderContext.renderChild]
...
> RenderTester<PropsT, StateT, OutputT, RenderingT>.expectWorkflow(
  ...
  expectWorkflow(
    ...
    assertProps = { @Suppress("UNCHECKED_CAST") assertProps(it as ChildPropsT) },
  )
 ...
 * @param outputTypeVerificationLevel The number of 'levels' of generic arguments to verify in the
 *   [OutputT], e.g., for `Wrapper<*>` and level 1 only `Wrapper` would be checked, whereas for
 ...
 *
 * This is a special use version for when the only reasonable [KClass] you have to verify against is
 * the definition of a workflow whose [OutputT] and [RenderingT] are supertypes of the [OutputT] and
 * [RenderingT] of the child workflow type you expect to be rendered. In other words, the expected
 * workflow is covariant with the class you have to pass to the expectation. (There is a slight
 * nuance here, in that if the [OutputT] is a supertype of the expected child's [OutputT], then
 * those workflow's are not actually covariant since [OutputT] is an invariant generic type. This is
 * not important for the use of this expectation, however.)
 *
 ...
 *
 * Note that this implementation does not handle
 * [ImpostorWorkflow][com.squareup.workflow1.ImpostorWorkflow]s (for proxied identifiers) like the
 * other versions do.
 */
public fun <ChildOutputT, ChildRenderingT, PropsT, StateT, OutputT, RenderingT> RenderTester<
  PropsT,
...
>
  ...
  expectWorkflow(
    ...
    val outputTypeMatches =
      invocation.outputType.type?.equals(childOutputType) == true ||
    ...
    val renderingTypeMatchers =
      invocation.renderingType.type?.equals(childRenderingType) == true ||
    ...
    if (childClassTypeMatches && keyMatches && outputTypeMatches && renderingTypeMatchers) {
      assertProps(invocation.props)
      ChildWorkflowMatch.Matched(rendering, output)
    } else {
      ChildWorkflowMatch.NotMatched
 ...
 *   this workflow.
 */
public fun <PropsT, StateT, OutputT, RenderingT> RenderTester<PropsT, StateT, OutputT, RenderingT>
  .expectSideEffect(key: String): RenderTester<PropsT, StateT, OutputT, RenderingT> =
 ...
 *
 * @param key The key passed to [remember][com.squareup.workflow1.BaseRenderContext.remember] when
 *   rendering this workflow.
 * @param resultType The type of the value returned by the `calculation` function passed to
...
public fun <PropsT, StateT, OutputT, RenderingT> RenderTester<PropsT, StateT, OutputT, RenderingT>
  ...
  return expectRemember(exactMatch = true, description = resolvedDescription) { invocation ->
    if (
      resultType.isSupertypeOf(invocation.resultType) &&
