 ...
 * [LEGACY_UNCONFINED] preserves the pre-1.25.1 behavior where coroutines dispatched immediately, so
 * existing tests continue to pass without changes. [VIRTUAL_TIME_STANDARD] opts in to the newer
 * [kotlinx.coroutines.test.StandardTestDispatcher] behavior where coroutines must be explicitly
 * advanced.
 *
...
public enum class DeprecatedLaunchSchedulerMode {
  /**
   * Uses [kotlinx.coroutines.test.UnconfinedTestDispatcher] — coroutines dispatch immediately. This
   * matches the behavior of the deprecated APIs before the 1.25.1 migration to
   * [kotlinx.coroutines.test.StandardTestDispatcher].
   */
  LEGACY_UNCONFINED,

  /**
   * Uses [kotlinx.coroutines.test.StandardTestDispatcher] — coroutines require explicit advancement
   * via `advanceUntilSettled()` or similar scheduler control.
   */
 ...
 *   helpers use. Defaults to [DeprecatedLaunchSchedulerMode.LEGACY_UNCONFINED] to preserve
 *   pre-1.25.1 behavior. Set to [DeprecatedLaunchSchedulerMode.VIRTUAL_TIME_STANDARD] to opt in to
 *   virtual-time semantics. Has no effect on the recommended `renderForTest` API. Note: if the
 *   `context` parameter passed to a `launchForTesting*` function contains a dispatcher, that
 *   dispatcher takes precedence over the one selected by this mode.
...
public class WorkflowTestParams<out StateT>(
  ...
  public sealed class StartMode<out StateT> {
     ...
     *
     * This differs from [StartFromCompleteSnapshot] because it represents only the snapshot for the
     * root workflow, without any of the snapshots of its children or other bookkeeping data added
     * by the workflow runtime.
     *
     * @param snapshot A [Snapshot] that can be directly parsed by a workflow's `initialState`
     *   method. For workflow trees, this is only the snapshot of the _root_ workflow, as returned
     *   by [snapshotState][com.squareup.workflow1.StatefulWorkflow.snapshotState]. To test with a
