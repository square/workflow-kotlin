 ...
 *   this workflow.
 * @param output If non-null, [WorkflowOutput.value] will be emitted when this worker is ran. The
 *   [WorkflowAction] used to handle this output can be verified using methods on
 *   [RenderTestResult].
 * @param description Optional string that will be used to describe this expectation in error
 *   messages.
 */
public inline fun <PropsT, StateT, OutputT, RenderingT> RenderTester<
  PropsT,
  StateT,
  OutputT,
  RenderingT,
 ...
 *   method to assert the worker matches.
 * @param key The key passed to [runningWorker][com.squareup.workflow1.runningWorker] when rendering
 *   this workflow.
 * @param output If non-null, [WorkflowOutput.value] will be emitted when this worker is ran. The
 *   [WorkflowAction] used to handle this output can be verified using methods on
...
public inline fun <
  ...
  RenderingT,
  WorkerOutputT,
  reified WorkerT : Worker<WorkerOutputT>,
...
> RenderTester<PropsT, StateT, OutputT, RenderingT>.expectWorker(
  ...
  expectWorker(
    ...
    key = key,
    assertWorker = { @Suppress("UNCHECKED_CAST") assertWorker(it as WorkerT) },
    output = output,
 ...
 *   an expected worker type of `typeOf<Worker<Collection<CharSequence>>>()` will match a worker
 *   that has the type `SomeConcreteWorker<List<String>>`.
 * @param key The key passed to [runningWorker][com.squareup.workflow1.runningWorker] when rendering
 *   this workflow.
 * @param assertWorker A function that will be passed the actual worker that matches this
 *   expectation and can perform custom assertions on the worker instance.
...
public fun <PropsT, StateT, OutputT, RenderingT> RenderTester<PropsT, StateT, OutputT, RenderingT>
  ...
  expectWorker(
    ...
    (key == actualKey &&
        (ruleExpectsNothing || !actualExpectsNothing) &&
        workerType.isSupertypeOf(actualWorkerType))
 ...
 *   exact matches, and the expectation will only be allowed to match a single worker. If false, the
 *   match will only be used if no other expectations return exclusive matches (in which case the
 *   first match will be used), and the expectation may match multiple workers.
 * @param predicate A function which receives the actual instance of the worker, and the key string,
 *   passed to [runningWorker][com.squareup.workflow.runningWorker], and returns true if the worker
 *   matches the expectation.
 */
internal fun <PropsT, StateT, OutputT, RenderingT> RenderTester<PropsT, StateT, OutputT, RenderingT>
  .expectWorker(
  description: String,
  output: WorkflowOutput<*>? = null,
  exactMatch: Boolean = true,
  predicate: (workerType: KType, worker: Worker<*>, key: String) -> Boolean,
): RenderTester<PropsT, StateT, OutputT, RenderingT> =
  expectWorkflow(description = description, exactMatch = exactMatch) { invocation ->
    val workerType =
      invocation.workflow.workerWorkflowWorkerTypeOrNull() ?: return@expectWorkflow NotMatched
...
@PublishedApi
internal fun keyDescription(key: String): String =
  key.takeUnless { it.isEmpty() }?.let { " with key \"$key\"" }.orEmpty()
