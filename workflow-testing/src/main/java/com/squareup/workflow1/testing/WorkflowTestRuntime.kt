...
internal constructor(
   ...
   * [WorkflowTestParams.autoAdvanceBeforeAwait] and [WorkflowTestParams.autoAdvanceBeforeHasCheck].
   * You only need to call this explicitly when you want to process pending actions when you are not
   * already awaiting a rendering or output — for example, to assert on side effects triggered by an
   * action that doesn't change state or produce output.
   *
   ...
   *
   * Note: if you trigger actions via callbacks on a rendering (not via [sendProps]), you still need
   * to call [advanceUntilSettled] explicitly (or await a rendering/output) before asserting.
   */
...
@Deprecated("Use renderForTest and WorkflowTurbine instead.")
@TestOnly
public fun <T, PropsT, OutputT, RenderingT> Workflow<PropsT, OutputT, RenderingT>
...
@Deprecated("Use renderForTest and WorkflowTurbine instead.")
@TestOnly
public fun <T, OutputT, RenderingT> Workflow<Unit, OutputT, RenderingT>
  .launchForTestingFromStartWith(
  testParams: WorkflowTestParams<Nothing> = WorkflowTestParams(),
  context: CoroutineContext = EmptyCoroutineContext,
...
/**
 * Creates a [WorkflowTestRuntime] to run this workflow for unit testing. If the workflow is
 * [stateful][StatefulWorkflow], [initialState][StatefulWorkflow.initialState] is not called.
 * Instead, the workflow is started from the given [initialState].
 *
 * All workflow-related coroutines are cancelled when the block exits.
 */
@Deprecated("Use renderForTest and WorkflowTurbine instead.")
@TestOnly
public fun <T, PropsT, StateT, OutputT, RenderingT> StatefulWorkflow<
  PropsT,
  StateT,
  OutputT,
  RenderingT,
...
>
  ...
  context: CoroutineContext = EmptyCoroutineContext,
  block: suspend WorkflowTestRuntime<PropsT, OutputT, RenderingT>.() -> T,
): T = launchForTestingWith(props, testParams.withStartState(initialState), context, block)
...
private fun <StateT> WorkflowTestParams<StateT>.withStartState(
  initialState: StateT
): WorkflowTestParams<StateT> =
  WorkflowTestParams(
...
>
  ...
  val schedulerContext =
    when (testParams.deprecatedLaunchSchedulerMode) {
