...
/**
 * Implementation of [Worker] for integration tests (using [launchForTestingFromStartWith] or
 * [launchForTestingFromStateWith]) that need to simply push values into the worker from the test.
 *
 ...
 */
public class WorkerSink<T>(private val name: String, private val type: KClass<*>) : Worker<T> {
  ...
  override fun doesSameWorkAs(otherWorker: Worker<*>): Boolean =
    otherWorker is WorkerSink && otherWorker.name == name && otherWorker.type == type
