...
class SnapshottingIntegrationTest {
  ...
  @Test
  fun `snapshots and restores single workflow`() {
  ...
  @Test
  fun `snapshots and restores parent child workflows`() {
    ...
    root.launchForTestingFromStartWith("initial props") {
      ...
      awaitNextRendering().setData("new root data")
  ...
  @Test
  fun `snapshots and restores complex tree`() {
    val root =
      TreeWorkflow(
        "root",
  ...
  fun `descendant snapshots are independent over state transitions`() {
    ...
    val root = Workflow.stateless<String, Nothing, () -> Unit> { renderChild(workflow, it) }
