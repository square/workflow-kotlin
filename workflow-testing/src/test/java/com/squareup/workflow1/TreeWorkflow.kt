 ...
 */
internal class TreeWorkflow(private val name: String, private vararg val children: TreeWorkflow) :
  ...
  class Rendering(
     ...
     *
     * The first argument is looked up in this.[children], the 2nd part, if any, is looked up in
     * that rendering's children, and so on.
     */
    operator fun get(vararg path: String): Rendering {
      return path.fold(this) { node, pathPart -> node.children.getValue(pathPart) }
    }
  }

  override fun initialState(props: String, snapshot: Snapshot?): String =
    snapshot?.bytes?.parse { it.readUtf8WithLength() } ?: props
  ...
  private fun onEvent(newState: String) = action("onEvent") { state = newState }
}
