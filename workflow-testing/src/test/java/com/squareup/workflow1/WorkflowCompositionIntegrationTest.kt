...
class WorkflowCompositionIntegrationTest {
  ...
  @Test
  fun `composes parent with single child`() {
  ...
  @Test
  fun `composes parent with multiple children`() {
    val root = TreeWorkflow("root", TreeWorkflow("leaf1"), TreeWorkflow("leaf2"))
  ...
  @Test
  fun `composes complex tree`() {
    val root =
      TreeWorkflow(
        "root",
        TreeWorkflow("middle1", TreeWorkflow("leaf1"), TreeWorkflow("leaf2")),
        TreeWorkflow("middle2", TreeWorkflow("leaf3")),
      )
  ...
  @Test
  fun `render fails when duplicate child key`() {
    val root = TreeWorkflow("root", TreeWorkflow("leaf"), TreeWorkflow("leaf"))

    // Setup initial state and change the state the workflow in the tree.
    assertFails { root.launchForTestingFromStartWith("initial props") { awaitNextRendering() } }
      .let { error ->
  ...
  @Test
  fun `renderChild closes over latest state`() {
