...
class LifecycleWorkerTest {
  ...
  @Test
  fun `onStart called immediately`() {
    ...
    runBlocking {
      val job = worker.run().launchIn(CoroutineScope(Unconfined))
  ...
  @Test
  fun `onCancelled called on cancel`() {
  ...
  @Test
  fun `doesSameWorkAs compares concrete types`() {
