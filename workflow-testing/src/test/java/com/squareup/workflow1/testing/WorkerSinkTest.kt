...
class WorkerSinkTest {
  ...
  @Test
  fun `workers are equivalent with matching type and name`() {
  ...
  @Test
  fun `workers are not equivalent with differing names`() {
    val fooWorker = WorkerSink<String>("foo")
  ...
  @Test
  fun `workers are not equivalent with differing types`() {
  ...
  @Test
  fun `values sent before running are received`() {
    val worker = WorkerSink<String>("foo")
    worker.send("hello")
    runBlocking { assertEquals("hello", worker.run().first()) }
  }

  @Test
  fun `values sent after running are received`() {
    val worker = WorkerSink<String>("foo")
    runBlocking {
      val deferred = async { worker.run().first() }
      yield()
      worker.send("hello")
  ...
  fun `multiple values are buffered`() {
    ...
    worker.send("world")
    worker.send("goodbye")
    runBlocking { assertEquals(listOf("hello", "world", "goodbye"), worker.run().take(3).toList()) }
  }
  ...
  fun `throws when consumed concurrently`() {
    ...
    runBlocking {
      launch(start = UNDISPATCHED) { worker.run().collect() }

      assertFailsWith<IllegalStateException> { worker.run().collect() }
  ...
  @Test
  fun `can be consumed multiple times sequentially`() {
    val worker = WorkerSink<String>("foo")
    worker.send("foo")
    worker.send("bar")
