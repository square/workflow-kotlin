...
internal class WorkflowTestRuntimeTest {
  ...
  @Test
  fun `propagates exception when block throws`() {
    val workflow = Workflow.stateless<Unit, Unit, Unit> {}

    rethrowingUncaughtExceptions {
      assertFailsWith<ExpectedException> {
        workflow.launchForTestingFromStartWith { throw ExpectedException() }
      }
    }
  }

  @Test
  fun `propagates exception when workflow throws from render`() {
    val workflow = Workflow.stateless<Unit, Unit, Unit> { throw ExpectedException() }
  ...
  @Test
  fun `propagates exception when workflow throws from initial state`() {
    val workflow =
      Workflow.stateful<Unit, Unit, Nothing, Unit>(
  ...
  @Test
  fun `propagates exception when workflow throws from snapshot state`() {
    val workflow =
  ...
  @Test
  fun `propagates exception when workflow throws from restore state`() {
    val workflow =
      Workflow.stateful<Unit, Nothing, Unit>(
  ...
  @Test
  fun `propagates exception when worker throws`() {
  ...
  @Test
  fun `does nothing when no outputs observed`() {
    val workflow = Workflow.stateless<Unit, Unit, Unit> {}
  ...
  @Test
  fun `workflow gets props from sendProps`() {
  ...
  @Test
  fun `sendProps duplicate values don't trigger render passes`() {
    var renders = 0
    val props = "props"
  ...
  @Test
  fun `fromStateWith props overload forwards testParams`() {
    var renderCount = 0
    val workflow =
      Workflow.stateful<String, Int, Nothing, Int>(
  ...
  @Test
  fun `fromStateWith unit overload forwards testParams`() {
    var renderCount = 0
    val workflow =
      Workflow.stateful<Unit, Int, Nothing, Int>(
        initialState = { _, _ -> fail("Should start from explicit initialState") },
        render = { _, state ->
          renderCount++
          state
  ...
  @Test
  fun `detects render side effects`() {
    var renderCount = 0
    val workflow = Workflow.stateless<Unit, Nothing, Unit> { renderCount++ }

    rethrowingUncaughtExceptions {
      workflow.launchForTestingFromStartWith { assertEquals(2, renderCount) }
    }
  }

  @Test
  fun `detects render side effects disabled`() {
    var renderCount = 0
    val workflow = Workflow.stateless<Unit, Nothing, Unit> { renderCount++ }
  ...
  @Test
  fun `uncaught exceptions are suppressed when test body throws`() {
    val workflow =
  ...
  @Test
  fun `exceptions from first render pass skip test body`() {
    val workflow = Workflow.stateless<Unit, Nothing, Unit> { throw ExpectedException("render") }

    rethrowingUncaughtExceptions {
      val firstError =
        assertFailsWith<ExpectedException> {
          workflow.launchForTestingFromStartWith { throw ExpectedException("test body") }
        }
  ...
  @Test
  fun `empty lambda still executes the workflow`() {
    var itLived = false
    val workflow = Workflow.stateless<Unit, Nothing, Unit> { itLived = true }
    workflow.launchForTestingFromStartWith {}
    assertTrue(itLived)
  }
}
