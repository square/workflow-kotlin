...
class WorkerRenderExpectationsTest {
  ...
  @Test
  fun `expectWorkerOutputting() works`() {
    val stringWorker =
    ...
    // Exact string match
    workflow.testRender(Unit).expectWorkerOutputting(typeOf<String>()).render()
    workflow
      .testRender(Unit)
    ...
    // Supertype match
    workflow.testRender(Unit).expectWorkerOutputting(typeOf<CharSequence>()).render()
    workflow
      .testRender(Unit)
      .expectWorkerOutputting(typeOf<String>(), output = WorkflowOutput("foo"))
      .render()
    ...
    // Other type match
    workflow.testRender(Unit).expectWorkerOutputting(typeOf<Int>()).render()
    workflow
      .testRender(Unit)
  ...
  @Test
  fun `expectWorker(worker instance) works`() {
    val stringWorker =
      object : Worker<String> {
        override fun run(): Flow<String> = emptyFlow()
      }
    ...
    // Exact string match
    workflow.testRender(Unit).expectWorker(stringWorker).render()
    workflow
      .testRender(Unit)
    ...
    // Other type match
    workflow.testRender(Unit).expectWorker(intWorker).render()
    workflow
      .testRender(Unit)
  ...
  @Test
  fun `expectWorker(worker class) works`() {
    ...
    // Exact string match
    workflow.testRender(Unit).expectWorker(EmptyStringWorker::class).render()
    workflow
      .testRender(Unit)
    ...
    workflow.testRender(Unit).expectWorker(EmptyWorker::class).render()
    workflow
      .testRender(Unit)
    ...
    workflow.testRender(Unit).expectWorker(EmptyWorkerCovariant::class).render()
    workflow
      .testRender(Unit)
    ...
    // Other type match
    workflow.testRender(Unit).expectWorker(EmptyIntWorker::class).render()
    workflow
      .testRender(Unit)
    ...
    Workflow.stateless<Unit, String, Unit> {
      ...
      .let {
        it.testRender(Unit).expectWorker(EmptyWorker::class).render()
        it
          .testRender(Unit)
          .expectWorker(EmptyWorker::class, output = WorkflowOutput("foo"))
          .render()
  ...
  @Test
  fun `expectWorker(worker KType) works`() {
    abstract class EmptyWorkerCovariant<out T> : Worker<T> {
      final override fun run(): Flow<T> = emptyFlow()
    }
    ...
    // Exact string match
    workflow.testRender(Unit).expectWorker(typeOf<EmptyStringWorker>()).render()
    workflow
      .testRender(Unit)
    ...
    // Supertype match without type args
    workflow.testRender(Unit).expectWorker(typeOf<EmptyWorker<*>>()).render()
    workflow
      .testRender(Unit)
    ...
    // Invariant supertype match with exact type args
    workflow.testRender(Unit).expectWorker(typeOf<EmptyWorker<String>>()).render()
    workflow
      .testRender(Unit)
    ...
    workflow.testRender(Unit).expectWorker(typeOf<EmptyWorkerCovariant<String>>()).render()
    workflow
      .testRender(Unit)
    ...
    workflow.testRender(Unit).expectWorker(typeOf<EmptyWorkerCovariant<CharSequence>>()).render()
    workflow
      .testRender(Unit)
    ...
    // Other type match
    workflow.testRender(Unit).expectWorker(typeOf<EmptyIntWorker>()).render()
    workflow
      .testRender(Unit)
    ...
    Workflow.stateless<Unit, String, Unit> {
      ...
      .let {
        it.testRender(Unit).expectWorker(typeOf<EmptyWorker<String>>()).render()
        it
          .testRender(Unit)
          .expectWorker(typeOf<EmptyWorker<String>>(), output = WorkflowOutput("foo"))
          .render()
  ...
  @Test
  fun `expectWorker(instance) considers doesSameWorkAs`() {
    ...
    // Matching input
    workflow.testRender(Unit).expectWorker(RequestWorker("foo")).render()

    // Non-matching input
    workflow.testRender(Unit).expectWorker(RequestWorker("bar")).apply {
      val error = assertFailsWith<AssertionError> { render() }
      assertEquals(
  ...
  @Test
  fun `expectWorker(instance) with default doesSameWorkAs() and different types`() {
    ...
    val multiWorkflow =
      Workflow.stateless<Unit, String, Unit> {
        ...
        //  renderTester is allowing duplicate workflows to be rendered. This test should fail
        // because
    ...
    trickWorkflow.testRender(Unit).expectWorker(honestWorker).apply {
      val error = assertFailsWith<AssertionError> { render() }
      assertEquals(
        "Expected actual worker's doesSameWorkAs to return true for expected worker \n" +
          "  expected=ParentWorker\n" +
    ...
    honestWorkflow.testRender(Unit).expectWorker(trickWorker).apply {
      val error = assertFailsWith<AssertionError> { render() }
      assertEquals(
        "Expected actual worker's doesSameWorkAs to return true for expected worker \n" +
  ...
  @Test
  fun `expectWorker on TypedWorker`() {
    ...
    workflow.testRender(Unit).expectWorker(workerType = typeOf<Worker<String>>()).render()
  }
}
