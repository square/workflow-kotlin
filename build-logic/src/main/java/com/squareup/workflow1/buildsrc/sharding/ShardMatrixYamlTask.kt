...
/**
 * This task manages test shard matrix configuration in a GitHub Actions workflow file, ensuring
 * that it matches the value of [numShards].
 *
 * @property yamlFile The CI configuration file this task works on.
 * @property startTagProperty The start tag to identify the matrix section in the CI configuration
 *   file.
 ...
 */
abstract class ShardMatrixYamlTask @Inject constructor(objectFactory: ObjectFactory) :
  DefaultTask() {
  /** kotlin.yml */
  @get:InputFile @get:PathSensitive(RELATIVE) val yamlFile = objectFactory.fileProperty()

  /**
   * Used to identify the start of the matrix. Everything after this tag and before the end tag will
   * be overwritten.
   *
  ...
  /** Used to identify the end of the matrix. ex: `### <end-my-shard-matrix>` */
  ...
  @get:Input abstract val numShards: Property<Int>

  /**
   * If true the file will be updated. If false, the task will fail if the matrix is out of date.
   */
  @get:Input abstract val autoCorrect: Property<Boolean>

  @get:Input abstract val updateTaskName: Property<String>

  private val matrixSectionRegex by
    lazy(NONE) {
  ...
  private fun requireCiFile(): File {
    ...
    require(ciFile.exists()) { "Could not resolve file: file://$ciFile" }
  ...
  companion object {
     ...
     * @param taskNamePart The part of the sharded task name which will be prepended to the matrix
     *   update task names.
    ...
    fun Project.registerYamlShardsTasks(
      ...
      require(yamlFile.exists()) { "Could not resolve '$yamlFile'." }

      val updateName = "${taskNamePart}ShardMatrixYamlUpdate"
      val updateTask =
        tasks.register(updateName, ShardMatrixYamlTask::class.java) { task ->
      ...
      val checkTask =
        tasks.register("${taskNamePart}ShardMatrixYamlCheck", ShardMatrixYamlTask::class.java) {
          task ->
          task.yamlFile.set(yamlFile)
          task.numShards.set(shardCount)
          task.startTagProperty.set(startTagName)
