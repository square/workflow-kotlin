 ...
 *   the interface [PoemWorkflow] had been left more flexible.
 *
...
class PerformancePoemWorkflow(
  ...
  override fun initialState(props: Poem, snapshot: Snapshot?): State {
    return if (simulatedPerfConfig.useInitializingState) {
      Initializing
    } else {
      Selected(NO_SELECTED_STANZA)
    }
  }
  ...
  override fun render(
    ...
    return when (renderState) {
      Initializing -> {
        ...
        context.runningWorker(Worker.from { isLoading.value = true }, "initializing") {
          action("initializing") {
        ...
        val (stanzaIndex, currentStateIsLoading, repeat) =
          when (renderState) {
        ...
        val previousStanzas: List<StanzaScreen> =
          if (stanzaIndex == NO_SELECTED_STANZA) {
            ...
            renderProps.stanzas
              ...
              .map { originalStanzaScreen -> originalStanzaScreen }
          }
