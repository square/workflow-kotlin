...
enum class BenchmarkTreeShape(
  ...
  ShallowBushyTree(degree = 100, depth = 1),
...
class WorkflowRuntimeMicrobenchmark(
  ...
  private fun benchmarkWorkflowStateChange(
    ...
    val resetStateAction = action<Any?, Int, Nothing>("resetState") {
      println("OMG in reset action")
      this.state = 0
    }

    benchmarkRule.measureRepeated {
      runWithTimingDisabled {
        println("OMG actionSinks=" + buildString {
          actionSinks.forEachIndexed { i, s ->
            append("[$i]=$s, ")
          }
        })
        ...
        actionSinks.forEachIndexed { index, sink ->
          println("OMG reseting state for leaf $index")
          sink!!.send(resetStateAction)
        }
        println("OMG running current after reset…")
        testScheduler.advanceUntilIdle()
        assertEquals(0, renderings.value.rendering)

        println("OMG setting states to test state…")
        ...
        testState { index, newState ->
          println("OMG sending action to set leaf[$index] to state $newState")
          actionSinks[index]!!.send(
            action<Any?, Int, Nothing>("setState") {
              println("OMG in action, setting leaf[$index] to state $newState")
              this.state = newState
            }
          )
        }
      ...
      // Run the render pass.
      println("OMG running current timed…")
      testScheduler.advanceUntilIdle()
      assertEquals(expectedTestRendering, renderings.value.rendering)
    }
...
private class BenchmarkWorkflowRoot(
  ...
  private inner class IntermediateChild : StatelessWorkflow<IntermediateProps, Nothing, Int>() {
    override fun render(
      ...
      // Formula for tree size: (degree^(depth+1) - 1) / (degree - 1)
      // val subtreeSize = ((degreeFloat.pow(subtreeDepth + 1) - 1) / (degreeFloat - 1)).toInt()
      val subtreeLeafCount = leafCount(degree = treeShape.degree, depth = subtreeDepth)
      var rendering = 0

      // if (renderingLeaves) {
      //   println("OMG at depth ${renderProps.depth} rendering leaves starting at ${renderProps.firstLeafIndex} with subtree depth $subtreeDepth and leaf count $subtreeLeafCount")
      // }
      ...
      repeat(treeShape.degree) { childIndex ->
        ...
        rendering += if (renderingLeaves) {
          if ((renderProps.renderFirstLeaf && firstLeafIndex == 0) ||
            ...
            // println("OMG rendering leaf child $childIndex with firstLeafIndex=$firstLeafIndex: $leafRendering")
            leafRendering
          } else {
            // println("OMG skipping rendering leaf child $childIndex with firstLeafIndex=$firstLeafIndex")
            0
          }
        } else {
          context.renderChild(
  ...
  private inner class Leaf : StatefulWorkflow<LeafProps, Int, Nothing, Int>() {
    ...
    override fun render(
      ...
      // println("OMG rendering leaf ${renderProps.index}")
      if (actionSinks != null) {
