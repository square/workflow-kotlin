...
internal class IdlingDispatcherTest {
  ...
  @Test
  fun `should not go to idle when dispatching to another dispatcher`() = runBlocking {
  ...
  @Test
  fun `should return to idle if a coroutine is cancelled before completion`() = runBlocking {
    val a = NamedDispatcher("a")
    val b = NamedDispatcher("b")
    ...
    val job =
      launch(idler) {
        withContext(b) {
          ...
          // because we need a suspension point in order to make the coroutine check for
          // cancellation.
          cancel()
          yield()
