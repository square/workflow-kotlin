...
/**
 * Creates and [remember]s a [ViewEnvironment] that has a special [ScreenViewFactoryFinder] and any
 * additional elements as configured by [viewEnvironmentUpdater].
 *
...
internal fun rememberPreviewViewEnvironment(
  ...
  val composableFactoryFinder =
    remember(mainFactory, placeholderModifier) {
      PreviewScreenComposableFactoryFinder(
        mainFactory,
        placeholderScreenComposableFactory(placeholderModifier),
      )
    }
  val screenFactoryFinder =
    remember(mainFactory, placeholderModifier) {
...
/**
 * A [ScreenComposableFactoryFinder] that uses [mainFactory] for rendering [RenderingT]s, and
 * [placeholderFactory] for all other
