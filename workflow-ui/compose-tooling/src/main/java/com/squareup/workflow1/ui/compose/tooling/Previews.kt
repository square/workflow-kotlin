...
/**
 * Uses [ScreenComposableFactory.Preview] or [ScreenViewFactory.Preview] to draw the receiving
 * [Screen].
 *
 * Note that this function can preview any kind of [Screen], whether it's bound to UI code
 * implemented via Compose or classic [View][android.view.View] code.
 *
...
public fun Screen.Preview(
  ...
  val factoryEnv = (viewEnvironmentUpdater?.invoke(ViewEnvironment.EMPTY) ?: ViewEnvironment.EMPTY)
...
/**
 * Like [ScreenComposableFactory.Preview], but for non-Compose [ScreenViewFactory] instances. Yes,
 * you can preview classic [View][android.view.View] code this way.
 *
 * Use inside `@Preview` Composable functions.
 *
 * *Note: [rendering] must be the same type as this [ScreenViewFactory], even though the type system
 * does not enforce this constraint. This is due to a Compose compiler bug tracked
 * [here](https://issuetracker.google.com/issues/156527332).*
 *
