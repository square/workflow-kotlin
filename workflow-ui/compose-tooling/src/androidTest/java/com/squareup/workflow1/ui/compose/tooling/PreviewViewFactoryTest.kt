...
internal class PreviewViewFactoryTest {
  ...
  @Test
  fun singleChild() {
    composeRule.setContent { ParentWithOneChildPreview() }
  ...
  @Test
  fun twoChildren() {
    composeRule.setContent { ParentWithTwoChildrenPreview() }
  ...
  @Test
  fun recursive() {
    composeRule.setContent { ParentRecursivePreview() }
  ...
  fun modifierIsApplied() {
    composeRule.setContent { ParentWithModifier() }
  ...
  @Test
  fun placeholderModifierIsApplied() {
    composeRule.setContent { ParentWithPlaceholderModifier() }
  ...
  @Test
  fun customViewEnvironment() {
    composeRule.setContent { ParentConsumesCustomKeyPreview() }
  ...
  data class TwoStrings(val first: Leaf, val second: Leaf) : Screen {
    constructor(first: String, second: String) : this(Leaf(first), Leaf(second))
  }

  data class ThreeStrings(val first: Leaf, val second: Leaf, val third: Leaf) : Screen {
    constructor(
      first: String,
      second: String,
      third: String,
  ...
  data class RecursiveRendering(val text: String, val child: RecursiveRendering? = null) : Screen

  private val ParentRecursive =
    ScreenComposableFactory<RecursiveRendering> { rendering ->
      Column {
        BasicText(rendering.text)
        rendering.child?.let { child -> WorkflowRendering(rendering = child) }
      }
    }
  ...
  private fun ParentRecursivePreview() {
    ParentRecursive.Preview(
      RecursiveRendering(
        text = "one",
        child = RecursiveRendering(text = "two", child = RecursiveRendering(text = "three")),
      )
    )
  }
  ...
  private fun ParentWithModifier() {
    ParentWithOneChild.Preview(TwoStrings("one", "two"), modifier = Modifier.size(0.dp))
  }

  @Preview
  @Composable
  private fun ParentWithPlaceholderModifier() {
    ParentWithOneChild.Preview(TwoStrings("one", "two"), placeholderModifier = Modifier.size(0.dp))
  }

  object TestEnvironmentKey : ViewEnvironmentKey<String>() {
    override val default: String
      get() = error("Not specified")
  }

  private val ParentConsumesCustomKey =
    ScreenComposableFactory<TwoStrings> { _ ->
