 ...
 * view of container root views (e.g. content views, e.g. backstack frames) so that when a view
 * inside a container calls [findViewTreeSavedStateRegistryOwner] on itself, one of these is
 * returned.
 *
 * The container should use a [WorkflowSavedStateRegistryAggregator] to manage its set of
 * [KeyedSavedStateRegistryOwner] instances, which will save and restore them via its own
 ...
 *   restored. They are driven by lifecycle signals, so refusing to advance until restoration makes
 *   it impossible for them to observe an unrestored registry.
 *
 ...
 * order, so a descendant's observers (e.g. a nested `ComposeView`'s composition) can run before the
 * aggregator's own restoration observer ever fires. To close that race, when [lifecycleOwner]
 ...
 */
internal class KeyedSavedStateRegistryOwner
