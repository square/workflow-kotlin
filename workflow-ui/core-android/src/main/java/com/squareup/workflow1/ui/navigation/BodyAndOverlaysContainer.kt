...
/**
 * Default container for [Overlay] renderings, providing support for orthogonal subtypes
 * [ModalOverlay] and [ScreenOverlay]. As much work as possible is delegated to the public
 * [LayeredDialogSessions] support class, to make it practical to write custom forks should the need
 * arise.
 */
internal class BodyAndOverlaysContainer
@JvmOverloads
constructor(
  context: Context,
  ...
  private val baseViewStub: WorkflowViewStub =
    WorkflowViewStub(context).apply {
      propagatesLayoutParams = false
      addView(this)
    }

  private val dialogs =
    LayeredDialogSessions.forView(
      view = this,
      superDispatchTouchEvent = { super.dispatchTouchEvent(it) },
    )

  fun update(newScreen: BodyAndOverlaysScreen<*, *>, viewEnvironment: ViewEnvironment) {
  ...
  companion object :
    ScreenViewFactory<BodyAndOverlaysScreen<*, *>> by ScreenViewFactory.fromCode(
      buildView = { _, initialEnvironment, context, _ ->
        BodyAndOverlaysContainer(context).let { view ->
