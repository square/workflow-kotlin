  ...
  internal fun updateLifecycle(isAttached: Boolean = view.isAttachedToWindow) {
    ...
    localLifecycle.currentState = when {
      ...
      if (newState == DESTROYED) {
        ...
        // TODO i think this is the wrong fix for a bug i'm not entirely sure about.
        // if (localState == INITIALIZED) {
        //   throw IllegalStateException("Can't transition from INITIALIZED to DESTROYED")
        //   // If we have observers that are in the INITIALIZED state, we can't move them directly to
        //   // DESTROYED (there's no event to do that), so move to CREATED first.
        //   localLifecycle.currentState = CREATED
        //   // Now after this also lambda returns, the state will be set to DESTROYED.
        // }
      }
    }
  }
