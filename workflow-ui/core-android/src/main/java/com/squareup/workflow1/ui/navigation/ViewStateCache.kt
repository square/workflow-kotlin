 ...
 *
 * - Provides [Parcelable]-based [save] and [restore] methods for use from a container's
 *   [View.onSaveInstanceState] and [View.onRestoreInstanceState] methods.
 * - Also handles androidx [SavedStateRegistryOwner] duties, via a wrapped instance of
 *   [WorkflowSavedStateRegistryAggregator]. This means that container views using this class must
 *   call [attachToParentRegistryOwner] and [detachFromParentRegistry] when they are
 *   [attached][View.onAttachedToWindow] and [detached][View.onDetachedFromWindow], respectively.
 */
...
internal constructor(
  ...
  /**
   * To be called when the set of hidden views changes but the visible view remains the same. Any
   * cached view state held for renderings that are not
  ...
  /**
   * **This method must be called on the main thread, before [newHolder]'s view is attached to a
   * window.** It installs a [SavedStateRegistryOwner] on that view via
   * [WorkflowSavedStateRegistryAggregator.installChildRegistryOwnerOn], which registers a lifecycle
   * observer.
   *
   * @param retainedRenderings the renderings to be considered hidden after this update. Any
   *   associated view state will be retained in the cache, possibly to be restored to the view of
   *   [newHolder] on a succeeding call to his method. Any other cached view state will be dropped.
   * @param oldHolderMaybe the view that is being removed, if any, which is expected to be showing a
   *   [NamedScreen] rendering. If that rendering is
   *   [compatible with][com.squareup.workflow1.ui.compatible] a member of [retainedRenderings], its
   *   state will be [saved][View.saveHierarchyState].
   * @param newHolder the view that is about to be displayed, which must be showing a [NamedScreen]
   *   rendering. If [compatible][com.squareup.workflow1.ui.compatible] view state is found in the
   *   cache, it is [restored][View.restoreHierarchyState].
  ...
  public fun update(
    ...
    if (oldHolderMaybe != null) {
      keyFor(oldHolderMaybe.showing)
        .takeIf { hiddenKeys.contains(it) }
        ?.let { savedKey ->
          // View state
          val saved =
            SparseArray<Parcelable>().apply { oldHolderMaybe.view.saveHierarchyState(this) }
   ...
   */
  public fun attachToParentRegistryOwner(key: String, parentOwner: SavedStateRegistryOwner) {
  ...
  /**
   * Replaces the state of the receiver with that of [from]. Typical usage is to call this from a
   * container view's [View.onRestoreInstanceState].
   */
  ...
  /**
   * Returns a [Parcelable] copy of the internal state of the receiver, for use with a container
   * view's [View.onSaveInstanceState].
   */
  ...
  public class Saved : Parcelable {
    ...
    public constructor(source: Parcel) {
      this.viewStates =
        mutableMapOf<String, ViewStateFrame>()
          .apply {
            ...
            if (VERSION.SDK_INT >= VERSION_CODES.TIRAMISU) {
              ...
              source.readMap(this as MutableMap<Any?, Any?>, ViewStateCache::class.java.classLoader)
            }
          }
          .toMap()
    }
    ...
    public companion object CREATOR : Creator<Saved> {
      override fun createFromParcel(source: Parcel): Saved = Saved(source)
