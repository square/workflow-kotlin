...
/**
 * Adds optional back button handling to a [content] rendering, possibly overriding that the wrapped
 * rendering's own back button handler.
 *
 * @param shadow If `true`, [onBackPressed] is set as the
 *   [backPressedHandler][android.view.View.setBackHandler] after the [content] rendering's view is
 *   built / updated, effectively overriding it. If false (the default), [onBackPressed] is set
 *   afterward, to allow the wrapped rendering to take precedence if it sets a `backPressedHandler`
 *   of its own -- the handler provided here serves as a default.
 * @param onBackPressed The function to fire when the device back button is pressed, or null to set
 *   no handler -- or clear a handler that was set previously. Defaults to `null`.
 */
