...
/**
 * Associates a [dialog] with a function ([runner]) that can update it to display instances of
 * [OverlayT]. Also holds a reference to the [ViewEnvironment][environment] that was most recently
 * used to update the [dialog].
 */
public interface OverlayDialogHolder<in OverlayT : Overlay> {
   ...
   * [ViewEnvironment].
   *
   * Prefer calling [show] to using this directly, to ensure that [overlayOrNull] is maintained
   * correctly, and [showing] keeps working. Or most commonly, allow `WorkflowViewStub.show` to call
   * it for you.
   */
  public val runner: (rendering: OverlayT, environment: ViewEnvironment) -> Unit

  /**
   * Optional function called to report the bounds of the managing container view, as reported by
   * [OverlayArea]. Well behaved [Overlay] dialogs are expected to be restricted to those bounds, to
   * the extent practical -- you probably want to ignore this for AlertDialog, e.g.
   *
   * Honoring this contract makes it easy to define areas of the display that are outside of the
   * "shadow" of a modal dialog. Imagine an app with a status bar that should not be covered by
   * modals.
   *
...
/**
 * Returns true if [overlay] is [compatible] with the [Overlay] instance that was last [shown][show]
 * by the [dialog][OverlayDialogHolder.dialog] managed by the receiver.
 */
...
/**
 * Updates the [dialog][OverlayDialogHolder.dialog] managed by the receiver to display [overlay],
 * and updates the receiver's [environment] as well.
 */
 ...
 *
 * Note that the exact type of the returned [Overlay] is likely not to match that of the receiver's
 * `OverlayT` type parameter, e.g. if a wrapping dialog factory is in use.
 */
