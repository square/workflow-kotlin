...
/**
 * Does the bulk of the work of maintaining a set of [Dialog][android.app.Dialog]s to reflect lists
 * of [Overlay], constrained to cover only the bounds of a single [View]. This is the work modeled
 * by [BodyAndOverlaysScreen] and executed by [BodyAndOverlaysContainer]. This class is public to
 * allow implementation of custom [Overlay]-based layouts if [BodyAndOverlaysContainer] is too
 * restrictive.
 *
 * - Provides an [allowBodyEvents] field that reflects the presence or absence of Dialogs driven by
 *   [ModalOverlay]
 *
 * - Makes [OverlayArea] available in the [ViewEnvironment], and uses it to drive calls to
 *   [OverlayDialogHolder.onUpdateBounds].
 *
 * - Provides a [LifecycleOwner] per managed Dialog, and view persistence support, both for classic
 *   [View.onSaveInstanceState] and Jetpack
 *   [SavedStateRegistry][androidx.savedstate.SavedStateRegistry].
 *
 ...
 * instance across multiple instances of [LayeredDialogSessions]. See [DialogCollator] for details.
 *
 * ## Lifecycle of a managed [Dialog][android.app.Dialog]
 *
 * When [update] is called with an [Overlay] that is not [Compatible] with an existing Dialog, the
 * appropriate [OverlayDialogFactory] instance is fetched from the [ViewEnvironment]. That factory
 * builds (but does not [show][android.app.Dialog.show]) a new Dialog, wrapped in an
 * [OverlayDialogHolder]. The holder in turn is held by a [DialogSession] instance. There is a 1:1:1
 * relationship between the Dialog, the [OverlayDialogHolder] which can
 * [update it][OverlayDialogHolder.show], and the [DialogSession] that manages its [LifecycleOwner]
 * and persistence.
 *
 ...
 *
 * - An appropriately scoped [SavedStateRegistry][androidx.savedstate.SavedStateRegistry] is put in
 *   place, provided that [onAttachedToWindow] and [onDetachedFromWindow] are called from the like
 *   named methods of the nearest container View
 *
 * - Any available classic View state is restored to the new Dialog's content View tree, provided
 *   that [onSaveInstanceState] and [onRestoreInstanceState] are called from the like named methods
 *   of that container View
 *
 * - And the Dialog is [shown][android.app.Dialog.show]
 *
 * The [DialogSession] is maintained (and its two flavors of View state are recorded as prompted by
 * the Android runtime) so long as [update] is called with a [Compatible] [Overlay] rendering.
 *
 * When [update] is called without a matching [Overlay], or the
 * [parent Lifecycle][getParentLifecycleOwner] ends, the [DialogSession] ends, its [LifecycleOwner]
 * is destroyed, and the Dialog is [dismissed][android.app.Dialog.dismiss].
 *
 * @param bounds made available to managed dialogs via the [OverlayArea]
 *   [ViewEnvironmentKey][com.squareup.workflow1.ui.ViewEnvironmentKey], which drives
 *   [OverlayDialogHolder.onUpdateBounds].
 * @param cancelEvents function to be called when a modal session starts -- that is, when [update]
 *   is first called with a [ModalOverlay] member, or called again with one after calls with none.
 * @param getParentLifecycleOwner provides the [LifecycleOwner] to serve as an ancestor to those
 *   created for managed [Dialog][android.app.Dialog]s.
 */
...
private constructor(
  ...
  /**
   * My unique ID in the [DialogCollator] shared with any other [LayeredDialogSessions] with a
   * shared ancestor.
   */
  private val idInCollator = UUID.randomUUID()

  /**
   * Provides a new `SavedStateRegistryOwner` for each dialog, which will save to the
   * `SavedStateRegistryOwner` of this container view.
   */
  ...
  /**
   * Reflects whether [onAttachedToWindow] has been called yet, so that we can avoid
   * [showing][DialogSession.showNewDialog] `Dialog` windows before the `Activity` window is ready.
   */
  private var attached = false

  /**
   * Set by an outermost instance only. If we are in a root [update] call before [attached] is true,
   * holds on to a list set of all [DialogSession]s, to be [shown][DialogSession.showNewDialog] at
   * attach time instead of immediately. Necessary to ensure that view state is restored in the
   * correct order -- the `Activity` `contentView` must be attached to its window before that of any
   * `Dialog`.
   *
   * Note that this is a superset of [sessions], also includes sessions built by any nested
   * [LayeredDialogSessions] instances during recursive [update] calls.
   *
   ...
   * [body][BodyAndOverlaysScreen.body] view beneath the [Dialog][android.app.Dialog] windows they
   * manage. It reflects that a [ModalOverlay] is (or soon will be) showing over the body.
   */
  ...
  /**
   * Updates the managed set of [Dialog][android.app.Dialog] instances to reflect [overlays]. Opens
   * new dialogs, updates existing ones, and dismisses those that match no member of that list.
   *
   * Each dialog has its own [WorkflowLifecycleOwner], which starts when the dialog is shown, and is
   * destroyed when it is dismissed. Views nested in a managed dialog can use
   * [LifecycleOwner][androidx.lifecycle.LifecycleOwner] as usual.
   *
   * @param updateBody function to be called before updating the dialogs, presumably to update the
   *   view for the [body][BodyAndOverlaysScreen.body] beneath the dialogs. The [ViewEnvironment]
   *   passed to the given function is enhanced with a [CoveredByModal] as appropriate, to ensure
  ...
  public fun update(
    ...
    val envWithDialogManager =
      viewEnvironment.establishDialogCollator(
        id = idInCollator,
        existingSessions = sessions,
        onRootUpdateFinished =
          if (attached) {
            null
          } else {
            { deferredShow = it }
    ...
    overlays.forEach { overlay ->
      updates +=
        DialogSessionUpdate(overlay) { oldSessions, covered ->
          ...
          oldSessionOrNull
            ...
            ?: overlay.toDialogFactory(dialogEnv).buildDialog(overlay, dialogEnv, context).let {
              holder ->
   ...
   *   Typically based on the [Compatible.keyFor] the current rendering. Taking this approach allows
   *   feature developers to take control over naming, e.g. by wrapping renderings with
   *   [NamedScreen][com.squareup.workflow1.ui.NamedScreen].
   */
  public fun onAttachedToWindow(savedStateParentKey: String, view: View) {
    attached = true
  ...
  public class SavedState : Parcelable {
    ...
    public constructor(source: Parcel) {
      dialogBundles =
        mutableListOf<KeyAndBundle>().apply { source.readTypedList(this, KeyAndBundle) }
    }
  ...
  public companion object {
    /**
     * Creates a [LayeredDialogSessions] instance based on the given [view], which will serve as the
     * source for a [LifecycleOwner], and whose bounds will be reported via [OverlayArea].
     *
     * - The [view]'s [dispatchTouchEvent][View.dispatchTouchEvent] and
     *   [dispatchKeyEvent][View.dispatchKeyEvent] methods should be overridden to honor
     *   [LayeredDialogSessions.allowBodyEvents].
     *
     * - The [view]'s [onAttachedToWindow][View.onAttachedToWindow] and
     *   [onDetachedFromWindow][View.onDetachedFromWindow] methods must call through to the like
     *   named methods of the returned [LayeredDialogSessions] ([onAttachedToWindow],
     *   [onDetachedFromWindow]).
     *
     * - The [view]'s [onSaveInstanceState][View.onSaveInstanceState] and
     *   [onRestoreInstanceState][View.onRestoreInstanceState] methods must call through to the like
     *   named methods of the returned [LayeredDialogSessions] ([onSaveInstanceState],
          ...
          val attachStateChangeListener =
            object : OnAttachStateChangeListener {
              val boundsListener = OnGlobalLayoutListener {
                ...
                // There are times that the reported bounds are above the insets or is negative ie)
                // when
                // the view is animating, and we don't want to accept those bounds.
                if (
                  boundsRect != boundsStateFlow.value &&
