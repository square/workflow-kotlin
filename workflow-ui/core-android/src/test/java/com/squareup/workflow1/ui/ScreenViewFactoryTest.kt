...
internal class ScreenViewFactoryTest {
  ...
  fun missingBindingMessage_isUseful() {
    ...
    val error =
      assertFailsWith<IllegalArgumentException> {
        fooScreen.toViewFactory(env).startShowing(fooScreen, env, mock())
      }
  ...
  @Test
  fun `buildView honors AndroidScreen`() {
  ...
  @Test
  fun `buildView prefers registry entries to AndroidViewRendering`() {
  ...
  @Test
  fun `convenience wrapper is convenient`() {
    val env = ViewEnvironment.EMPTY + ViewRegistry()
  ...
  private class MyWrapper<C : Screen>(override val content: C) :
  ...
  private class TestViewFactory<T : Screen>(override val type: KClass<in T>) :
    ScreenViewFactory<T> {
    var built = false
    var updated = false
    ...
    override fun buildView(
      ...
      return ScreenViewHolder(initialEnvironment, mock()) { _, _ -> updated = true }
    }
  }
