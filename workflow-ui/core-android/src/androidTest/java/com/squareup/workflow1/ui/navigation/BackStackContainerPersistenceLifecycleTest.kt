...
import com.squareup.workflow1.ui.navigation.fixtures.viewForScreen
import com.squareup.workflow1.ui.navigation.fixtures.waitForScreen
...
/**
 * Uses a custom subclass, [NoTransitionBackStackContainer], to ensure transitions are synchronous.
 */
internal class BackStackContainerPersistenceLifecycleTest {

  private val scenarioRule = ActivityScenarioRule(BackStackContainerLifecycleActivity::class.java)
  ...
  @Test
  fun restores_view_on_pop_without_config_change() {
    val firstScreen = LeafRendering("initial")
    lateinit var firstView: View

    scenario.onActivity { it.update(firstScreen) }
    ...
    scenario.onActivity { it.update(firstScreen, LeafRendering("new screen")) }
   ...
   */
  @Test
  fun state_restores_after_recreate_without_crashing() {
    ...
    scenario.onActivity {
      ...
      assertThat(it.consumeLifecycleEvents())
        .containsExactly("activity onCreate", "activity onStart", "activity onResume")
        .inOrder()
    }
  ...
  @Test
  fun restores_current_view_after_config_change() {
    val firstScreen = LeafRendering("initial")
    ...
    // Check that the view state was actually restored.
    scenario.viewForScreen(("initial")).let { assertThat(it.viewState).isEqualTo("hello world") }
  }

  @Test
  fun restores_view_on_pop_after_config_change() {
    val firstScreen = LeafRendering("initial")
  ...
  @Test
  fun state_rendering_and_attach_ordering() {
    val firstRendering = LeafRendering("first")
    val secondRendering = LeafRendering("second")

    // Setup initial screen.
    scenario.onActivity { it.update(firstRendering) }
    ...
    // Navigate forward.
    scenario.onActivity { it.update(firstRendering, secondRendering) }
   ...
   */
  @Test
  fun lifecycle_stop_then_resume() {
    assertThat(scenario.state).isEqualTo(RESUMED)
    scenario.onActivity { it.update(LeafRendering("initial")) }

    scenario.onActivity { activity ->
      assertThat(activity.consumeLifecycleEvents())
        .containsAtLeast(
          "activity onCreate",
          "activity onStart",
          "activity onResume",
  ...
  @Test
  fun lifecycle_recreate_rendering() {
    assertThat(scenario.state).isEqualTo(RESUMED)
    scenario.onActivity { it.update(LeafRendering("initial")) }
  ...
  @Test
  fun lifecycle_recreate_activity() {
    ...
    scenario.recreate()
    scenario.onActivity { it.update(LeafRendering("recreated")) }

    scenario.onActivity {
      assertThat(initialActivity.consumeLifecycleEvents())
        .containsAtLeast(
          "LeafView initial ON_PAUSE",
          "activity onPause",
          "LeafView initial ON_STOP",
  ...
  fun lifecycle_replace_screen() {
    ...
    scenario.onActivity { it.update(LeafRendering("next")) }
  ...
  @Test
  fun lifecycle_nested_lifecycle() {
    assertThat(scenario.state).isEqualTo(RESUMED)
    scenario.onActivity {
      it.consumeLifecycleEvents()
    ...
    scenario.onActivity { it.update(LeafRendering("unwrapped")) }
  ...
  fun lifecycle_is_destroyed_when_navigating_forward() {
    ...
    scenario.onActivity { it.update(LeafRendering("first"), LeafRendering("second")) }
  ...
  @Test
  fun lifecycle_is_destroyed_when_navigating_backwards() {
    assertThat(scenario.state).isEqualTo(RESUMED)
    scenario.onActivity {
      it.consumeLifecycleEvents()
      it.update(LeafRendering("first"), LeafRendering("second"))
    }
    ...
    scenario.onActivity { it.update(LeafRendering("first")) }
