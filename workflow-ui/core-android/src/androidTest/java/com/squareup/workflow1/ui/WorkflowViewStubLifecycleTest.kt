...
internal class WorkflowViewStubLifecycleTest {
  ...
  private val scenarioRule = ActivityScenarioRule(WorkflowViewStubLifecycleActivity::class.java)
  ...
  @Test
  fun restores_registry_state_on_subview_after_config_change() {
    scenario.onActivity { it.update(CounterRendering()) }
    ...
    onView(withTagValue(equalTo(CounterRendering.Tag))).check(matches(withText("Counter: 1")))
  }

  @Test
  fun propagates_savedstateregistryowner_to_subviews() {
    val expectedRegistryOwner =
    ...
    data class RegistrySetter(val wrapped: TestRendering) : ViewRendering<RegistrySetter>() {
      override val viewFactory =
        fromCode<RegistrySetter> { _, initialEnvironment, context, _ ->
                      ...
                      override fun onStateChanged(source: LifecycleOwner, event: Event) {
                        if (event == ON_CREATE) {
                          ...
                          registryOwner.savedStateRegistry
                            .consumeRestoredStateForKey("counter")
                            ?.let { restoredState ->
