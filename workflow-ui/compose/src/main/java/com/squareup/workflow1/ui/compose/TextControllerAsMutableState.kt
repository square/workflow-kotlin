...
/**
 * Exposes the [textValue][TextController.textValue] of a [TextController] as a remembered
 * [MutableState], suitable for use from `@Composable` functions.
 *
 * Usage:
 *
 * var text by rendering.textController.asMutableState()
 *
 * OutlinedTextField( label = {}, placeholder = { Text("Enter some text") }, value = text,
...
public fun TextController.asMutableState(): MutableState<String> {
  ...
  return remember(this) { mutableStateOf(textValue) }
    .also { state ->
      // …and to restart the effect.
      LaunchedEffect(this) {
        // Push changes from the workflow to the state.
        launch { onTextChanged.collect { state.value = it } }
...
/**
 * Exposes the [textValue][TextController.textValue] of a [TextController] as a remembered
 * [MutableState] of [TextFieldValue], suitable for use from `@Composable` functions.
 *
 * Usage:
 *    ```
 ...
 *   [TextRange.end], then nothing is selected, and the cursor is placed at [TextRange.start]. By
 *   default, the cursor will be placed at the end of the text.
 */
@Composable
public fun TextController.asMutableTextFieldValueState(
  ...
  LaunchedEffect(this) {
    launch {
      ...
      // By subscribing directly to `onTextChanged` we can use this to also update the
      // textFieldValue.
      onTextChanged.collect { newText ->
    ...
    snapshotFlow { textFieldValue.value }.collect { newText -> textValue = newText.text }
  }
