...
internal class WorkflowRenderingTest {
  ...
  @Test fun incompatibleLegacyViewsHaveDistinctSavedStateRegistryOwners() {
    ...
    composeRule.runOnIdle {
      assertThat(log.owners).containsKey("outgoing")
    }
  ...
  @Test fun siblingLegacyViewsHaveDistinctSavedStateRegistryOwners() {
    val log = RegistryRegistrationLog()
    ...
    composeRule.setContent {
      env.RootScreen(SiblingHost())
    }

    composeRule.runOnIdle {
      assertThat(log.failures).isEmpty()
      assertThat(log.owners.keys).containsExactly("first", "second")
      assertThat(log.owners.getValue("first"))
        .isNotSameInstanceAs(log.owners.getValue("second"))
    }
  }

  @Test fun composableRenderingKeepsParentSavedStateRegistryOwner() {
    lateinit var parentOwner: SavedStateRegistryOwner
    ...
    composeRule.runOnIdle {
      assertThat(renderingOwner).isSameInstanceAs(parentOwner)
    }
  }
