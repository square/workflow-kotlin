...
internal class ComposeViewTreeIntegrationTest {
  ...
  @Test
  fun compose_view_assertions_work() {
    val firstScreen = VanillaComposeRendering("first") { BasicText("First Screen") }
    val secondScreen = VanillaComposeRendering("second") {}

    scenario.onActivity { it.setBackstack(firstScreen) }

    composeRule.onNodeWithText("First Screen").assertIsDisplayed()

    // Navigate away from the first screen.
    scenario.onActivity { it.setBackstack(firstScreen, secondScreen) }

    composeRule.onNodeWithText("First Screen").assertDoesNotExist()
  }

  @Test
  fun composition_is_disposed_when_navigated_away_stock_class() {
    var composedCount = 0
    var disposedCount = 0
    val firstScreen =
  ...
  @Test
  fun composition_is_disposed_when_navigated_away_default_strategy() {
    var composedCount = 0
    var disposedCount = 0
    val firstScreen =
  ...
  @Test
  fun composition_is_disposed_when_navigated_away_dispose_on_detach_strategy() {
    var composedCount = 0
    var disposedCount = 0
    val firstScreen =
  ...
  @Test
  fun composition_is_disposed_when_navigated_away_dispose_on_destroy_strategy() {
    var composedCount = 0
    var disposedCount = 0
    val firstScreen =
  ...
  @Test
  fun composition_state_is_restored_after_config_change() {
    val firstScreen =
      VanillaComposeRendering("first") {
        var counter by rememberSaveable { mutableStateOf(0) }
        BasicText("Counter: $counter", Modifier.clickable { counter++ }.testTag(CounterTag))
      }
  ...
  @Test
  fun composition_state_is_restored_after_navigating_back() {
    val firstScreen =
      VanillaComposeRendering("first") {
        var counter by rememberSaveable { mutableStateOf(0) }
        BasicText("Counter: $counter", Modifier.clickable { counter++ }.testTag(CounterTag))
      }
    val secondScreen = VanillaComposeRendering("second") { BasicText("nothing to see here") }
    ...
    composeRule.onNodeWithTag(CounterTag).assertDoesNotExist()
  ...
  fun composition_state_is_restored_after_config_change_then_navigating_back() {
    ...
    composeRule.onNodeWithText("nothing to see here").assertIsDisplayed()
  ...
  @Test
  fun composition_state_is_not_restored_after_screen_is_removed_from_backstack() {
    val firstScreen =
      VanillaComposeRendering("first") {
        var counter by rememberSaveable { mutableStateOf(0) }
        BasicText("Counter: $counter", Modifier.clickable { counter++ }.testTag(CounterTag))
    ...
    // Show first screen to initialize state.
    scenario.onActivity { it.setBackstack(firstScreen) }
    composeRule.onNodeWithTag(CounterTag).assertTextEquals("Counter: 0").performClick()
    ...
    scenario.onActivity { it.setBackstack(secondScreen) }

    // Navigate to the first screen again.
    scenario.onActivity { it.setBackstack(firstScreen) }

    composeRule.onNodeWithTag(CounterTag).assertTextEquals("Counter: 0")
  }
  ...
  @Test
  fun composition_is_restored_in_overlay_after_config_change() {
    val firstScreen: Screen =
      VanillaComposeRendering(compatibilityKey = "") {
        var counter by rememberSaveable { mutableStateOf(0) }
        BasicText("Counter: $counter", Modifier.clickable { counter++ }.testTag(CounterTag))
      }
  ...
  @Test
  fun composition_is_restored_in_multiple_overlays_after_config_change() {
    val firstScreen: Screen =
      VanillaComposeRendering(compatibilityKey = "0") {
        var counter by rememberSaveable { mutableStateOf(0) }
        BasicText("Counter: $counter", Modifier.clickable { counter++ }.testTag(CounterTag))
      }

    val secondScreen: Screen =
      VanillaComposeRendering(compatibilityKey = "1") {
        var counter by rememberSaveable { mutableStateOf(0) }
        BasicText("Counter2: $counter", Modifier.clickable { counter++ }.testTag(CounterTag2))
      }

    val thirdScreen: Screen =
      VanillaComposeRendering(compatibilityKey = "2") {
        var counter by rememberSaveable { mutableStateOf(0) }
        BasicText("Counter3: $counter", Modifier.clickable { counter++ }.testTag(CounterTag3))
      }
    ...
    scenario.onActivity {
      it.setRendering(
        BodyAndOverlaysScreen(
          EmptyRendering,
          listOf(TestOverlay(firstScreen), TestOverlay(secondScreen), TestOverlay(thirdScreen)),
        )
      )
    }
    ...
    composeRule.onNodeWithTag(CounterTag2).assertTextEquals("Counter2: 1")

    composeRule.onNodeWithTag(CounterTag3).assertTextEquals("Counter3: 1")
  }

  @Test
  fun composition_is_restored_in_multiple_overlays_backstacks_after_config_change() {
    fun createRendering(layer: Int, screen: Int) =
      VanillaComposeRendering(
        // Use the same compatibility key across layers – these screens are in different overlays,
        // so
        // they won't conflict.
        ...
        BasicText(
          ...
          Modifier.clickable { counter++ }.testTag("L${layer}S$screen"),
        )
      }
    ...
    composeRule.onNodeWithTag("L0S0").assertDoesNotExist()
    composeRule.onNodeWithTag("L1S0").assertDoesNotExist()
    ...
    composeRule.onNodeWithTag("L0S1").assertIsDisplayed()
    composeRule.onNodeWithTag("L1S1").assertIsDisplayed()
    ...
    composeRule.onNodeWithText("Counter[0][0]: 1").assertIsDisplayed()
    composeRule.onNodeWithText("Counter[1][0]: 1").assertIsDisplayed()
  }

  @Test
  fun composition_handles_overlay_reordering() {
    val composeA: Screen =
      VanillaComposeRendering(compatibilityKey = "0") {
        var counter by rememberSaveable { mutableStateOf(0) }
        BasicText("Counter: $counter", Modifier.clickable { counter++ }.testTag(CounterTag))
    ...
    scenario.onActivity {
      it.setRendering(
        BodyAndOverlaysScreen(
          EmptyRendering,
          listOf(TestOverlay(EmptyRendering), TestOverlay(composeB), TestOverlay(composeA)),
        )
      )
    }
  ...
  @Test
  fun composition_under_view_stub_handles_overlay_reordering() {
    val composeA: Screen =
      VanillaComposeRendering(compatibilityKey = "0") {
        var counter by rememberSaveable { mutableStateOf(0) }
        BasicText("Counter: $counter", Modifier.clickable { counter++ }.testTag(CounterTag))
  ...
  data class TestOverlay(override val content: Screen) :
  ...
  data class ViewStubWrapper<C : Screen>(override val content: C) :
    ...
    override val viewFactory: ScreenViewFactory<ViewStubWrapper<C>> =
      fromCode { _, initialEnvironment, context, _ ->
        ...
        FrameLayout(context)
          .apply { this.addView(stub) }
          .let { ScreenViewHolder(initialEnvironment, it) { r, e -> stub.show(r.content, e) } }
      }
  }
  ...
  data class BespokeComposeRendering(
    ...
    Compatible, AndroidScreen<BespokeComposeRendering>, ScreenViewFactory<BespokeComposeRendering> {
    override val type: KClass<in BespokeComposeRendering> = BespokeComposeRendering::class
    override val viewFactory: ScreenViewFactory<BespokeComposeRendering>
      get() = this
  ...
  /**
   * Bog standard [ComposeScreen], as opposed to [BespokeComposeRendering]. Requires
