...
internal class ComposeViewTreeIntegrationTest {
  ...
  @Test fun android_view_saved_state_is_restored_after_config_change() {
    val child = SavedStateAndroidRendering("child")

    scenario.onActivity {
      it.setRendering(ComposeAndroidHost(child))
    }
    composeRule.runOnIdle {
    ...
    composeRule.runOnIdle {
      assertThat(child.restoredValues).containsExactly(0, 41).inOrder()
    }
  }
  ...
  private data class ComposeAndroidHost(
    val child: Screen
  ) : ComposeScreen {
  ...
  private class SavedStateAndroidRendering(
    ...
    override val viewFactory =
      fromCode<SavedStateAndroidRendering> { rendering, initialEnvironment, context, _ ->
        val view = SavedStateView(context) { restoredValue ->
          rendering.restoredValues += restoredValue
        }
        rendering.view = view
