...
internal class BackStackScreenTest {
  ...
  @Test
  fun `top  is last`() {
    assertThat(BackStackScreen(FooScreen(1), FooScreen(2), FooScreen(3), FooScreen(4)).top)
      .isEqualTo(FooScreen(4))
  }

  @Test
  fun `backstack is all but top`() {
  ...
  @Test
  fun `get works`() {
    assertThat(BackStackScreen(FooScreen("able"), FooScreen("baker"), FooScreen("charlie"))[1])
      .isEqualTo(FooScreen("baker"))
  }

  @Test
  fun `plus another stack`() {
    val sum =
      BackStackScreen("fred", FooScreen(1), FooScreen(2), FooScreen(3)) +
  ...
  @Test
  fun `unequal have mismatching hash`() {
  ...
  /**
   * To reminds us why we want the `out` in `BackStackScreen<out T : Screen>`. Without this, using
   * `BackStackScreen<*>` as `RenderingT` is not practical.
   */
  @Test
  fun heterogeneousPlusIsTolerable() {
