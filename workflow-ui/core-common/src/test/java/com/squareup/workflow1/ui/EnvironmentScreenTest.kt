...
internal class EnvironmentScreenTest {
  private class TestFactory<T : Any>(type: KClass<in T>) : ViewRegistry.Entry<T> {
  ...
  private data class TestValue(val value: String) {
    companion object : ViewEnvironmentKey<TestValue>() {
      override val default: TestValue
        get() = error("Set a default")
    }
  }
  ...
  @Test
  fun `Screen withEnvironment works`() {
    val fooFactory = TestFactory(FooScreen::class)
    val viewRegistry = ViewRegistry(fooFactory)
    val envScreen = FooScreen.withEnvironment(EMPTY + viewRegistry + TestValue("foo"))
    ...
    assertThat(
      ...
      .isNull()
    assertThat(envScreen.environment[TestValue]).isEqualTo(TestValue("foo"))
  }

  @Test
  fun `EnvironmentScreen withRegistry merges`() {
    ...
    assertThat(union.environment[ViewRegistry].getFactoryFor<FooScreen, TestFactory<*>>(FooScreen))
      .isSameInstanceAs(fooFactory2)

    assertThat(union.environment[ViewRegistry].getFactoryFor<BarScreen, TestFactory<*>>(BarScreen))
      .isSameInstanceAs(barFactory)
  }

  @Test
  fun `EnvironmentScreen withEnvironment merges`() {
    val fooFactory1 = TestFactory(FooScreen::class)
    val fooFactory2 = TestFactory(FooScreen::class)
    val barFactory = TestFactory(BarScreen::class)

    val left =
      FooScreen.withEnvironment(EMPTY + ViewRegistry(fooFactory1, barFactory) + TestValue("left"))

    val union = left.withEnvironment(EMPTY + ViewRegistry(fooFactory2) + TestValue("right"))

    assertThat(union.environment[ViewRegistry].getFactoryFor<FooScreen, TestFactory<*>>(FooScreen))
      .isSameInstanceAs(fooFactory2)
    assertThat(union.environment[ViewRegistry].getFactoryFor<BarScreen, TestFactory<*>>(BarScreen))
      .isSameInstanceAs(barFactory)
    assertThat(union.environment[TestValue]).isEqualTo(TestValue("right"))
  }

  @Test
  fun `keep existing instance on vacuous merge`() {
