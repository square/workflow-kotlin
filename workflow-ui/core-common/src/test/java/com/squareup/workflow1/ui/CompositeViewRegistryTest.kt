...
internal class CompositeViewRegistryTest {
  ...
  fun `constructor throws on duplicates`() {
    ...
    val error = assertFailsWith<IllegalArgumentException> { fooBarRegistry + barBazRegistry }
    assertThat(error).hasMessageThat().startsWith("Must not have duplicate entries: ")
    assertThat(error).hasMessageThat().contains(BarRendering::class.java.name)
  }

  @Test
  fun `getFactoryFor delegates to composite registries`() {
    ...
    val fooBarRegistry =
      TestRegistry(mapOf(fooFactory.key to fooFactory, barFactory.key to barFactory))
  ...
  @Test
  fun `getFactoryFor returns null on missing registry`() {
