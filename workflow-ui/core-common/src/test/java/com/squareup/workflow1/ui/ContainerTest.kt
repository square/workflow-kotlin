...
class ContainerTest {
  ...
  private data class TestScreenContainer<T : Screen>(val children: List<T>) :
  ...
  private data class TestScreenOverlay<S : Screen>(override val content: S) : ScreenOverlay<S> {
    override fun <ContentU : Screen> map(transform: (S) -> ContentU) =
  ...
  @Test
  fun `can unwrap through BodyAndOverlaysScreen to Body`() {
  ...
  fun `can unwrap through BodyAndOverlaysScreen to an Overlay`() {
    val container =
      BodyAndOverlaysScreen(body = TestScreen(), overlays = listOf(TestOverlay(0), TestOverlay(1)))
