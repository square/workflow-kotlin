...
object NestedOverlaysWorkflow : StatefulWorkflow<Unit, State, Nothing, Screen>() {
  ...
  override fun render(
    ...
    val toggleTopBarButton =
      Button(
        name = if (renderState.showTopBar) R.string.hide_top else R.string.show_top,
        onClick =
          context.eventHandler("show / hide top") {
            state = state.copy(showTopBar = !state.showTopBar)
          },
      )

    val toggleBottomBarButton =
      Button(
        name = if (renderState.showBottomBar) R.string.hide_bottom else R.string.show_bottom,
        onClick =
          context.eventHandler("show / hide bottom") {
            state = state.copy(showBottomBar = !state.showBottomBar)
          },
      )

    val outerSheet =
      if (!renderState.showOuterSheet) {
        null
      } else {
        val closeOuter =
          context.eventHandler("closeOuter") { state = state.copy(showOuterSheet = false) }
        FullScreenModal(
          BackButtonScreen(
            ButtonBar(
              Button(name = R.string.close, onClick = closeOuter),
    ...
    val innerSheet =
      if (!renderState.showInnerSheet) {
        null
      } else {
        val closeInner =
          context.eventHandler("closeInner") { state = state.copy(showInnerSheet = false) }
        FullScreenModal(
          BackButtonScreen(
            ButtonBar(
              Button(name = R.string.close, onClick = closeInner),
              toggleTopBarButton,
    ...
    return BodyAndOverlaysScreen(
      ...
      body =
        TopAndBottomBarsScreen(
          topBar =
            if (!renderState.showTopBar) {
              null
            } else {
              context.topBottomBar(top = true, renderState)
            },
          content =
          ...
          bottomBar =
            if (!renderState.showBottomBar) {
              null
            } else {
              context.topBottomBar(top = false, renderState)
            },
        ),
    )
  }
  ...
  private fun RenderContext<Unit, State, Nothing>.topBottomBar(
    ...
    return ButtonBar(
      toggleInnerSheetButton(name = name, renderState = renderState),
      Button(
        name = R.string.cover_all,
        onClick =
          eventHandler("$name cover everything") { state = state.copy(showOuterSheet = true) },
      ),
    )
  }
  ...
  private fun RenderContext<Unit, State, Nothing>.toggleInnerSheetButton(
    ...
    Button(
      ...
      onClick =
        eventHandler("$name: reveal / cover body") {
