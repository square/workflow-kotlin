...
Return this output from a new `onBack` action.
...
It's worth taking a step back to think about what that `BackStackScreen` is doing,
...
There is no `BackStackWorkflow`, there are `push`, `pop` or `goto` methods,
...
Our experience with captial "N" Navigation Frameworks (including the first one that we wrote)
has been that their model is not ours,
and that as our apps scale to not-even-all-that-big
we find ourselves needing to engineer around the framework instead of just building our apps.
...
- collects their `Screen` renderings in a widget (`BackStackScreen`) which is:
  - good at recognizing if the stack of renderings it's managing just grew or shrank
  - playing a push or a pop animation accordingly
  - while also taking care to save and restore view system state of pushed and popped screens
...
> It's also very good at window management.
