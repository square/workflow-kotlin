...
@Composable
fun SampleLauncherApp() {
  MaterialTheme(colors = darkColors()) {
    Scaffold(topBar = { TopAppBar(title = { Text(stringResource(R.string.app_name)) }) }) { padding
      ->
      LazyColumn(contentPadding = padding) { items(samples) { sample -> SampleItem(sample) } }
    }
  }
}
...
@Composable
private fun SamplePreview(sample: Sample, onPreviewCoordinates: (LayoutCoordinates) -> Unit) {
...
private fun launchSample(sample: Sample, rootView: View, sourceBounds: Rect?) {
