...
/**
 * A simple workflow that produces [Rendering]s of zero or more children. The rendering provides
 * event handlers for adding children and resetting child count to zero.
 *
...
object RecursiveWorkflow : StatefulWorkflow<Unit, State, Unit, Screen>() {
  ...
  data class LegacyRendering(val rendering: Screen) : AndroidScreen<LegacyRendering> {
    override val viewFactory =
      ScreenViewFactory.fromViewBinding(LegacyViewBinding::inflate, ::LegacyRunner)
  }
  ...
  private fun addChild() = action("addChild") { state = state.copy(children = state.children + 1) }

  private fun resetChildren() =
    action("resetChildren") { state = state.copy(children = 0, pendingFlashes = emptyList()) }

  private fun flashImmediately(id: Int? = null) =
    action("triggerImmediateFlash[id=$id]") {
      val newPendingFlashes = if (id == null) state.pendingFlashes else state.pendingFlashes - id
      state = state.copy(flashTrigger = state.flashTrigger + 1, pendingFlashes = newPendingFlashes)
      setOutput(Unit)
    }

  private fun flashAfterDelay() =
    action("triggerDelayedFlash") {
