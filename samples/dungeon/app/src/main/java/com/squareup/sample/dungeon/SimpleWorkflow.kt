...
class SimpleWorkflow : StatefulWorkflow<Props, String, Nothing, Screen>() {
  override fun initialState(props: Props, snapshot: Snapshot?): String =
    snapshot?.bytes?.parse { it.readUtf8() } ?: "initial"
...
private data class MyScreen(val text: String, val onClick: () -> Unit) : ComposeScreen {
  @Composable
  override fun Content() {
    Modifier.pointerInput(Unit) { extendedTouchPadding }
    BasicText(
      text = text,
      color = { Color.White },
      modifier =
