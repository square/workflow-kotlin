...
class GameSessionWorkflow(
  ...
  data class Props(val boardPath: BoardPath, val paused: Boolean = false)
  ...
  override fun initialState(props: Props, snapshot: Snapshot?): State = Loading
  ...
  override fun render(
    ...
    when (renderState) {
      ...
      is Running -> {
        ...
        val gameScreen =
          context.renderChild(gameWorkflow, gameInput) { handleGameOutput(it, renderState.board) }
  ...
  private fun handleGameOutput(output: GameWorkflow.Output, board: Board) =
