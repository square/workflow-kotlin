 ...
 *
 * When [recording][RecordValue], the [RecordValue.value] will be recorded at the current time and
 ...
 */
@ExperimentalTime
internal class RecorderWorkflow<T>(private val clock: TimeSource) :
  ...
  data class Recording<T>(val startTime: TimeMark, val series: TimeSeries<T>)

  override fun initialState(props: RecorderProps<T>, snapshot: Snapshot?): Recording<T> {
  ...
  override fun render(
    ...
    return TimeMachineRendering(value = value, totalDuration = renderState.series.duration)
  }
