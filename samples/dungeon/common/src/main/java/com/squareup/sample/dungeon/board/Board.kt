 ...
 */
@Serializable data class BoardMetadata(val name: String)
...
data class Board(
  ...
  operator fun get(x: Int, y: Int): BoardCell = cells[cellIndexOf(x, y)]
  ...
  fun withOverlay(actors: Map<Location, BoardCell>): Board {
    ...
    return copy(cells = cells.mapIndexed { index, cell -> playersByIndex[index] ?: cell })
  }

  override fun toString(): String {
    return cells.asSequence().chunked(width).joinToString(separator = "\n") {
      it.joinToString(separator = "")
    }
  }

  private fun cellIndexOf(x: Int, y: Int): Int = (y * width) + x

  companion object {
    /** Builds a board from a square list of [BoardCell] lists. */
    fun fromRows(metadata: BoardMetadata, rows: List<List<BoardCell>>): Board {
      val width =
        rows.map { it.size }.distinct().singleOrNull()
