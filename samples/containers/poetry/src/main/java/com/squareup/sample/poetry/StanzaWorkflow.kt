...
object StanzaWorkflow : StatelessWorkflow<Props, Output, StanzaScreen>() {
  data class Props(val poem: Poem, val index: Int, val eventHandlerTag: (String) -> String = { it })
  ...
  override fun render(renderProps: Props, context: RenderContext<Props, Output>): StanzaScreen {
    with(renderProps) {
      val onGoBack: (() -> Unit)? =
        when (index) {
          0 -> null
