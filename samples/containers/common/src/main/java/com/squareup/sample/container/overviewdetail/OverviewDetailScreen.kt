 ...
 *
 * @param selectDefault optional function that a split view container may call to request that a
 *   selection be made to fill a null [detailRendering]. This function _must_ perform an action that
 *   leads to an updated rendering with either a non-null [detailRendering], or a null
 *   [selectDefault]. **[selectDefault] cannot be a no-op.**
 */
class OverviewDetailScreen<out T : Screen>
private constructor(
  ...
  /**
   * @param selectDefault optional function that a split view container may call to request that a
   *   selection be made to fill a null [detailRendering].
   */
  constructor(
    overviewRendering: BackStackScreen<T>,
  ...
  /**
   * For nicer logging. See the call to [unwrap][com.squareup.workflow1.ui.unwrap] in the activity.
   */
 ...
 * [detailRendering][OverviewDetailScreen.detailRendering] of [other] to those of the receiver. If
 * the new screen's `detailRendering` is `null`, it will have the
...
operator fun <T : Screen> OverviewDetailScreen<T>.plus(
  ...
  val newDetail = detailRendering?.let { it + other.detailRendering } ?: other.detailRendering
