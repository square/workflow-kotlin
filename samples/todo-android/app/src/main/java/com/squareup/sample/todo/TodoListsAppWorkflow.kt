...
sealed class TodoListsAppState {
  ...
  data class EditingList(override val lists: List<TodoList>, val editingIndex: Int) :
    TodoListsAppState()
}

/**
 * Renders a [TodoListsWorkflow] and a [TodoEditorWorkflow] in a overview / detail relationship. See
 * details in the body of the [render] method.
 */
object TodoListsAppWorkflow :
  StatefulWorkflow<Unit, TodoListsAppState, Nothing, OverviewDetailScreen<*>>() {
  override fun initialState(props: Unit, snapshot: Snapshot?): TodoListsAppState =
    ShowingLists(listOf(TodoList("Groceries"), TodoList("Daily Chores"), TodoList("Reminders")))

  private val listsWorkflow = TodoListsWorkflow()
  private val editorWorkflow = TodoEditorWorkflow()

  private fun onListSelected(index: Int) =
    action("onListSelected") { state = EditingList(state.lists, index) }

  private fun onEditOutput(output: TodoEditorOutput) =
    action("onEditOutput") {
      state =
        when (output) {
          is ListUpdated -> {
            val oldState = state as EditingList
            oldState.copy(lists = state.lists.updateRow(oldState.editingIndex, output.newList))
          }
  ...
  override fun render(
    ...
    val listOfLists: TodoListsScreen =
      context.renderChild(listsWorkflow, renderState.lists) { index -> onListSelected(index) }
      ...
      is EditingList ->
        context
          ...
          .let { editScreen ->
            OverviewDetailScreen(
              overviewRendering =
                BackStackScreen(listOfLists.copy(selection = renderState.editingIndex)),
...
private fun <T> List<T>.updateRow(index: Int, newValue: T): List<T> =
  withIndex().map { (i, value) -> if (i == index) newValue else value }
