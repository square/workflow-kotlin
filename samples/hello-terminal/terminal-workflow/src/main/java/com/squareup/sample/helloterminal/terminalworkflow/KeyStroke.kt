 ...
 */
data class KeyStroke(val character: Char?, val keyType: KeyType) {
  enum class KeyType {
    Backspace,
    Character,
    ArrowUp,
...
internal fun InputProvider.listenForKeyStrokesOn(scope: CoroutineScope): SharedFlow<KeyStroke> {
  return flow {
...
private fun LanternaKeystroke.toKeyStroke(): KeyStroke =
  KeyStroke(
    character = character,
    keyType =
      when (keyType) {
