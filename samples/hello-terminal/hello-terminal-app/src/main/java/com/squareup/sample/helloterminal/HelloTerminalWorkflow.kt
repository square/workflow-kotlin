...
class HelloTerminalWorkflow :
  TerminalWorkflow, StatefulWorkflow<TerminalProps, State, ExitCode, TerminalRendering>() {
  ...
  override fun initialState(props: TerminalProps, snapshot: Snapshot?) = State()
  ...
  override fun render(
    ...
    return TerminalRendering(text = header + prompt + renderState.text + cursor, textColor = GREEN)
  }

  override fun snapshotState(state: State): Snapshot? = null

  private fun onKeystroke(key: KeyStroke): HelloTerminalAction =
    action("onKeystroke") {
      when {
