...
class TodoWorkflow :
  TerminalWorkflow, StatefulWorkflow<TerminalProps, TodoList, ExitCode, TerminalRendering>() {
  ...
  data class TodoList(
    ...
    fun toggleChecked(index: Int) =
      copy(
        items = items.mapIndexed { i, item -> item.copy(checked = item.checked xor (index == i)) }
      )
  ...
  data class TodoItem(val label: String, val checked: Boolean = false)

  override fun initialState(props: TerminalProps, snapshot: Snapshot?) =
    TodoList(
      title = "Grocery list",
      items = listOf(TodoItem("eggs"), TodoItem("cheese"), TodoItem("bread"), TodoItem("beer")),
    )
  ...
  private fun onKeystroke(key: KeyStroke) =
    action("onKeystroke") {
      when (key.keyType) {
...
private fun updateTitle(newTitle: String): TodoAction =
  action("updateTitle") { state = state.copy(title = newTitle) }

private fun setLabel(index: Int, text: String): TodoAction =
  action("setLabel") {
    state =
      state.copy(
        items =
          state.items.mapIndexed { i, item -> if (index == i) item.copy(label = text) else item }
      )
  }
...
private val TodoList.titleSeparator
  get() = "–".repeat(title.length + 1)
...
private fun renderSelection(text: String, selected: Boolean): String {
