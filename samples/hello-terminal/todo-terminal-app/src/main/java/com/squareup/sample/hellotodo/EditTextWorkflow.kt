...
class EditTextWorkflow : StatefulWorkflow<EditTextProps, EditTextState, String, String>() {
  ...
  data class EditTextProps(val text: String, val terminalProps: TerminalProps)
   ...
   */
  data class EditTextState(val cursorPosition: Int)

  override fun initialState(props: EditTextProps, snapshot: Snapshot?) =
  ...
  override fun render(
    ...
    context.runningWorker(renderProps.terminalProps.keyStrokes) { key -> onKeystroke(key) }
...
private fun moveCursor(text: String, state: EditTextState, delta: Int): EditTextState =
...
private fun String.insertCharAt(index: Int, char: Char): String =
  substring(0, index) + char + substring(index, length)
