...
internal class WorkflowIdentifierTest {
  ...
  @Test
  fun restored_identifier_toString() {
  ...
  @Test
  fun flat_identifiers_for_same_class_are_equal() {
  ...
  @Test
  fun flat_identifiers_for_different_classes_are_not_equal() {
    val id1 = TestWorkflow1.identifier
  ...
  @Test
  fun impostor_identifiers_for_same_proxied_class_are_equal() {
  ...
  @Test
  fun impostor_identifiers_for_different_proxied_classes_are_not_equal() {
    val impostorId1 = TestImpostor1(TestWorkflow1).identifier
  ...
  @Test
  fun different_impostor_identifiers_for_same_proxied_class_are_not_equal() {
    val impostorId1 = TestImpostor1(TestWorkflow1).identifier
  ...
  @Test
  fun identifier_restored_from_source_is_equal_to_itself() {
    val id = TestWorkflow1.identifier
    val serializedId = id.toByteStringOrNull()!!
    val restoredId = WorkflowIdentifier.parse(serializedId)
  ...
  @Test
  fun identifier_restored_from_source_is_not_equal_to_different_identifier() {
    val id1 = TestWorkflow1.identifier
    val id2 = TestWorkflow2.identifier
  ...
  @Test
  fun impostor_identifier_restored_from_source_is_equal_to_itself() {
  ...
  @Test
  fun read_from_empty_source_throws() {
    assertFailsWith<IllegalArgumentException> { WorkflowIdentifier.parse(ByteString.EMPTY) }
  }

  @Test
  fun read_from_invalid_source_throws() {
    val source = Buffer().apply { writeUtf8("invalid data") }.readByteString()
    assertFailsWith<IllegalArgumentException> { WorkflowIdentifier.parse(source) }
  }

  @Test
  fun read_from_corrupted_source_throws() {
    val source = TestWorkflow1.identifier.toByteStringOrNull()!!.toByteArray()

    val corruptedSource =
      Buffer().apply { write(source.dropLast(2).toByteArray()) }.readByteString()

    assertFailsWith<IllegalArgumentException> { WorkflowIdentifier.parse(corruptedSource) }
  }

  @Test
  fun unsnapshottable_identifier_returns_null_ByteString() {
    val id = unsnapshottableIdentifier(typeOf<TestWorkflow1>())
    assertNull(id.toByteStringOrNull())
  }

  @Test
  fun unsnapshottable_identifiers_for_same_class_are_equal() {
  ...
  @Test
  fun unsnapshottable_identifiers_for_different_class_are_not_equal() {
    val id1 = unsnapshottableIdentifier(typeOf<String>())
  ...
  @Test
  fun unsnapshottable_impostor_identifier_returns_null_ByteString() {
    val id = TestUnsnapshottableImpostor(typeOf<String>()).identifier
    assertNull(id.toByteStringOrNull())
  }

  @Test
  fun impostor_of_unsnapshottable_impostor_identifier_returns_null_ByteString() {
    val id = TestImpostor1(TestUnsnapshottableImpostor(typeOf<String>())).identifier
    assertNull(id.toByteStringOrNull())
  }

  @Test
  fun getRealIdentifierType_returns_self_for_non_impostor_workflow() {
    val id = TestWorkflow1.identifier
    assertEquals(Snapshottable(TestWorkflow1::class), id.realType)
  }

  @Test
  fun getRealIdentifierType_returns_real_identifier_for_impostor_workflow() {
    val id = TestImpostor1(TestWorkflow1).identifier
    assertEquals(Snapshottable(TestWorkflow1::class), id.realType)
  }

  @Test
  fun getRealIdentifierType_returns_leaf_real_identifier_for_impostor_workflow_chain() {
    val id = TestImpostor2(TestImpostor1(TestWorkflow1)).identifier
    assertEquals(Snapshottable(TestWorkflow1::class), id.realType)
  }

  @Test
  fun getRealIdentifierType_returns_KType_of_unsnapshottable_identifier() {
  ...
  public class TestImpostor1(private val proxied: Workflow<*, *, *>) :
  ...
  public class TestImpostor2(proxied: Workflow<*, *, *>) :
    Workflow<Nothing, Nothing, Nothing>, ImpostorWorkflow {
    override val realIdentifier: WorkflowIdentifier = proxied.identifier
  ...
  public class TestUnsnapshottableImpostor(type: KType) :
    Workflow<Nothing, Nothing, Nothing>, ImpostorWorkflow {
