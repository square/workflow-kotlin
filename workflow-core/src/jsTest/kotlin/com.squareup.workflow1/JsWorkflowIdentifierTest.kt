...
internal class JsWorkflowIdentifierTest {
  ...
  fun impostor_identifier_toString_uses_full_chain_when_describeRealIdentifier_returns_null() {
    ...
    val idStructure =
      RegExp("WorkflowIdentifier\\(TestImpostor\\((\\d)+\\), TestWorkflow1\\((\\d)+\\)\\)")
    assertTrue(idStructure.test(id.toString()))
  }

  @Test
  fun impostor_identifier_description() {
  ...
  @Test
  fun same_workflow_returns_same_identifier() {
  ...
  @Test
  fun different_workflows_same_namespace() {
    val id1 = JsMockWorkflow1().identifier
  ...
  @Test
  fun same_workflow_name_different_namespace() {
    val id1 = JsMockWorkflow1().identifier
