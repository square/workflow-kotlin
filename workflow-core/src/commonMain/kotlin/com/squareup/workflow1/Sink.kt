 ...
 *
 * Another way to think about this is: `map` transforms a type by changing the output types of its
 * API, while `contraMap` transforms a type by changing the *input* types of its API.
 */
...
/**
 * Collects from a [Flow] by converting each item into a [WorkflowAction] and then sending them to
 * the [actionSink]. This may be used as an alternative to a [asWorker] for certain [Flow]s.
 *
 ...
 * > state or props. The side effect will capture those values when it first runs, and if the
 * > instance of the flow changes in a future render pass, the side effect will still be collecting
 * > the stale [Flow]. In this case
 */
 ...
 * has been invoked. Since a [Sink] may be backed by an unbounded buffer, this method can be used to
 * apply back pressure to the caller when there are a lot events being sent to the workflow runtime.
 *
 ...
 */
internal suspend fun <PropsT, StateT, OutputT> Sink<WorkflowAction<PropsT, StateT, OutputT>>
  .sendAndAwaitApplication(action: WorkflowAction<PropsT, StateT, OutputT>) {
