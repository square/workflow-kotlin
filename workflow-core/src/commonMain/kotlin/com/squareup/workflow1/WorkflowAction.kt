 ...
 *
 * A [WorkflowAction]'s [apply] method is executed in the context of an
 * [Updater][WorkflowAction.Updater], which provides access to the current
 * [props][WorkflowAction.Updater.props] and [state][WorkflowAction.Updater.state], along with a
 * [setOutput][WorkflowAction.Updater.setOutput] function. The [state][WorkflowAction.Updater.state]
 * can be updated with a new [StateT] instance that will become the current one after the [apply]
 * function finishes.
 *
...
public abstract class WorkflowAction<PropsT, StateT, OutputT> {
  ...
  /**
   * The name to use for debugging. This is handy for logging and is used by the default [toString]
   ...
   */
  public inner class Updater(public val props: PropsT, public var state: StateT) {
    internal val startingState = state
    internal var outputOrNull: WorkflowOutput<OutputT>? = null
      private set

    /**
     * Sets the value the workflow will emit as output when this action is applied. If this method
     * is not called, there will be no output.
     */
  ...
  public companion object {
    /**
     * Returns a [WorkflowAction] that does nothing: no output will be emitted, and the state will
     * not change.
     *
    ...
    private val NO_ACTION =
      object : WorkflowAction<Any?, Any?, Any?>() {
...
/**
 * Creates a [WorkflowAction] from the [apply] lambda. The returned object will include the string
 * returned from [name] in its [toString].
 *
 * It is more common to use [StatefulWorkflow.action] or [StatelessWorkflow.action] instead of this
 * function directly, to avoid repeating its parameter types.
 *
...
public class WorkflowOutput<OutputT>(public val value: OutputT) {
 ...
 *
 * Only [ActionApplied] needs the generic OutputT so we do not include it in the root interface
 * here.
 */
 ...
 *   action. Otherwise it is a [WorkflowOutput] around the output value of type [OutputT], which
 *   could be null.
 ...
 */
public class ActionApplied<OutputT>
@JvmOverloads
constructor(public val output: WorkflowOutput<OutputT>?, public val stateChanged: Boolean = false) :
