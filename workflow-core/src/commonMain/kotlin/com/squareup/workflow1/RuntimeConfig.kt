...
public enum class RuntimeConfigOptions {
  /**
   * If state has not changed from an action cascade (as determined via `equals()`), do not
   * re-render. For example, when this is selected and `noAction()` is enqueued, the current
   * `render()` pass will short circuit and no rendering will be posted through the `StateFlow`
   * returned from `renderWorkflowIn()`.
   *
   * This has been mostly proven out. However, be careful if you have any non-Workflow code you
   * integrate with that depends on the Workflow tree re-rendering to pick up changes from its
   * equivalent 'view model.' You should change some kind of Workflow state when updating that
   * external code if you want Workflow to pick up the change and render again.
   */
  @WorkflowExperimentalRuntime RENDER_ONLY_WHEN_STATE_CHANGES,
   ...
   *
   * Note however that you must be careful using this because there may be external state that your
   * Workflow draws in and re-renders, and if that is not explicitly tracked within that Workflow's
   * state then the Workflow will not re-render. In this case make sure that the implicit state is
   * tracked within the Workflow's `StateT` in some way, even if only via a hash token.
   */
  @WorkflowExperimentalRuntime PARTIAL_TREE_RENDERING,
   ...
   * handling of an action happens synchronously after the coroutine sending it into the sink is
   * resumed. This means it never dispatches multiple coroutines that send actions into the sink
   ...
   */
  @WorkflowExperimentalRuntime CONFLATE_STALE_RENDERINGS,

  /**
   * Changes the default value of the `remember: Boolean?` parameter of `RenderContext.eventHandler`
   * calls from `false` to `true`.
   */
  @WorkflowExperimentalRuntime STABLE_EVENT_HANDLERS,
   ...
   */
  @WorkflowExperimentalRuntime WORK_STEALING_DISPATCHER,

  /**
   * If we have more actions to process that are queued on nodes not affected by the last action
   * application, then we will continue to process those actions before another render pass.
   *
   * Note that this is *not effective* when using an immediate or unconfined dispatcher as the
   * handling of an action happens synchronously after the coroutine sending it into the sink is
   * resumed. This means it never dispatches multiple coroutines that send actions into the sink
   ...
   * `AndroidUiDispatcher.Main` in order to access these optimizations.
   */
  @WorkflowExperimentalRuntime DRAIN_EXCLUSIVE_ACTIONS,
  ...
  public companion object {
    /**
     * Baseline configuration where we render for each action and always pass the rendering to the
     * view layer.
     */
    ...
    enum class RuntimeOptions(val runtimeConfig: RuntimeConfig) {
      ...
      RENDER_ONLY_PARTIAL_TREE_CONFLATE(
        setOf(RENDER_ONLY_WHEN_STATE_CHANGES, PARTIAL_TREE_RENDERING, CONFLATE_STALE_RENDERINGS)
      ),
      ...
      RENDER_ONLY_PARTIAL_TREE_STEAL(
        setOf(RENDER_ONLY_WHEN_STATE_CHANGES, PARTIAL_TREE_RENDERING, WORK_STEALING_DISPATCHER)
      ),
      ...
      RENDER_ONLY_CONFLATE_STEAL(
        setOf(RENDER_ONLY_WHEN_STATE_CHANGES, CONFLATE_STALE_RENDERINGS, WORK_STEALING_DISPATCHER)
      ),
      RENDER_ONLY_STABLE_DRAIN(
        setOf(RENDER_ONLY_WHEN_STATE_CHANGES, STABLE_EVENT_HANDLERS, DRAIN_EXCLUSIVE_ACTIONS)
      ),
      RENDER_ONLY_STABLE_STEAL(
        setOf(RENDER_ONLY_WHEN_STATE_CHANGES, STABLE_EVENT_HANDLERS, WORK_STEALING_DISPATCHER)
      ),
      RENDER_ONLY_DRAIN_STEAL(
        setOf(RENDER_ONLY_WHEN_STATE_CHANGES, DRAIN_EXCLUSIVE_ACTIONS, WORK_STEALING_DISPATCHER)
      ),
      PARTIAL_TREE_CONFLATE_STABLE(
        setOf(PARTIAL_TREE_RENDERING, CONFLATE_STALE_RENDERINGS, STABLE_EVENT_HANDLERS)
      ),
      PARTIAL_TREE_CONFLATE_DRAIN(
        setOf(PARTIAL_TREE_RENDERING, CONFLATE_STALE_RENDERINGS, DRAIN_EXCLUSIVE_ACTIONS)
      ),
      PARTIAL_TREE_CONFLATE_STEAL(
        setOf(PARTIAL_TREE_RENDERING, CONFLATE_STALE_RENDERINGS, WORK_STEALING_DISPATCHER)
      ),
      PARTIAL_TREE_STABLE_DRAIN(
        setOf(PARTIAL_TREE_RENDERING, STABLE_EVENT_HANDLERS, DRAIN_EXCLUSIVE_ACTIONS)
      ),
      PARTIAL_TREE_STABLE_STEAL(
        setOf(PARTIAL_TREE_RENDERING, STABLE_EVENT_HANDLERS, WORK_STEALING_DISPATCHER)
      ),
      PARTIAL_TREE_DRAIN_STEAL(
        setOf(PARTIAL_TREE_RENDERING, DRAIN_EXCLUSIVE_ACTIONS, WORK_STEALING_DISPATCHER)
      ),
      CONFLATE_STABLE_DRAIN(
        setOf(CONFLATE_STALE_RENDERINGS, STABLE_EVENT_HANDLERS, DRAIN_EXCLUSIVE_ACTIONS)
      ),
      CONFLATE_STABLE_STEAL(
        setOf(CONFLATE_STALE_RENDERINGS, STABLE_EVENT_HANDLERS, WORK_STEALING_DISPATCHER)
      ),
      CONFLATE_DRAIN_STEAL(
        setOf(CONFLATE_STALE_RENDERINGS, DRAIN_EXCLUSIVE_ACTIONS, WORK_STEALING_DISPATCHER)
      ),
      STABLE_DRAIN_STEAL(
        setOf(STABLE_EVENT_HANDLERS, DRAIN_EXCLUSIVE_ACTIONS, WORK_STEALING_DISPATCHER)
      ),
