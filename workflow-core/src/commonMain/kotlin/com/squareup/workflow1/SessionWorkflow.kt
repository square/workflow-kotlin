...
/**
 * An extension of [StatefulWorkflow] that gives [initialState] a [CoroutineScope] that corresponds
 * with the lifetime of _session_ driven by this Workflow.
 *
 ...
 * of [initialState] that does not have the [CoroutineScope] must also be implemented as it is an
 * abstract fun, even though it would never be used. With this version, that confusion is removed
 * and only the version of [initialState] with the [CoroutineScope] must be implemented.
 */
@WorkflowExperimentalApi
abstract class SessionWorkflow<PropsT, StateT, OutputT, out RenderingT> :
   ...
   *
   * - own the transforms on a [StateFlow][kotlinx.coroutines.flow.StateFlow], linking them to the
   *   lifetime of a Workflow session. For example, here is how you might safely combine two
   *   `StateFlow`s:
   *
   *         ```
   *         data class MyState(
   ...
   *         ```
   *
   *     - set reliable teardown hooks, e.g. via
   *       [Job.invokeOnCompletion][kotlinx.coroutines.Job.invokeOnCompletion]. Note however, that
   *       while these are reliable in the sense of being guaranteed to be executed regardless of
   *       the lifetime of this workflow session, they are not reliable in that a completion handler
   *       on the Job is not thread-safe and will be executed on whatever the last dispatcher was
   *       used when the Job completes. See more on the
   *       [Job.invokeOnCompletion][kotlinx.coroutines.Job.invokeOnCompletion] kdoc.
   *
   * So what do you do? Well, cleanup and lifecycle matters should be handled by each individual
   * Worker and sideEffect. Consider using a try { } finally { cleanup() } or
   ...
   * and should not have its own state. Instead, the transformed/computed dependencies must be put
   * into the [StateT] of this Workflow in order to be properly maintained.
   *
   ...
   * [CoroutineScope]
   */
  public final override fun initialState(props: PropsT, snapshot: Snapshot?): StateT {
...
public inline fun <PropsT, StateT, OutputT, RenderingT> Workflow.Companion.sessionWorkflow(
  ...
  crossinline render:
    BaseRenderContext<PropsT, StateT, OutputT>.(props: PropsT, state: StateT) -> RenderingT,
...
public inline fun <StateT, OutputT, RenderingT> Workflow.Companion.sessionWorkflow(
  ...
  crossinline snapshot: (StateT) -> Snapshot?,
): SessionWorkflow<Unit, StateT, OutputT, RenderingT> =
  sessionWorkflow(
