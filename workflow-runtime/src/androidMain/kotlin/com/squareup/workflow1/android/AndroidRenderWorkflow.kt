...
/**
 * An Android `ViewModel`-friendly wrapper for [com.squareup.workflow1.renderWorkflowIn], for use
 * with a [workflow] that takes no input (that is, has `PropsT` set to [Unit]).
 *
 * **WARNING** Note that this function returns a `StateFlow<RenderingT>`, and bear in mind that
 * `StateFlow` swallows updates to new values that are [equals] to the current one -- which is
 * likely to be the case most of the time if your root `RenderingT` implementation is a `data
 * class`.
 *
 *    ```
 ...
 * @param scope The [CoroutineScope] in which to launch the workflow runtime, typically from the
 *   androidx `ViewModel.viewModelScope` extension. Any exceptions thrown in any workflows, after
 ...
 *   be used to execute the very first render pass (which happens synchronously). We recommend using
 *   a `CoroutineDispatcher` in this scope's `CoroutineContext` that will ensure that all dispatched
 *   coroutines are run before the next Choreographer frame. Compose UI's `AndroidUiDispatcher.Main`
 *   provides this behavior in a performant way! Another way to achieve that is to use an immediate
 *   (or Unconfined) dispatcher like Dispatchers.Main.immediate. However, that dispatcher cannot
 *   take advantage of some runtime optimizations. E.G., see
 *   [RuntimeConfigOptions.DRAIN_EXCLUSIVE_ACTIONS].
 * @param savedStateHandle Used to restore workflow state in a new process. Typically this is the
 *   `savedState: SavedStateHandle` constructor parameter of an androidx
 ...
 *   propagate backpressure if used to forward outputs to a [Flow][kotlinx.coroutines.flow.Flow] or
 *   [Channel][kotlinx.coroutines.channels.Channel], for example.
 * @param runtimeConfig Configuration for the Workflow Runtime.
 * @return A [StateFlow] of [RenderingT]s that will emit any time the root workflow creates a new
 *   rendering.
 */
...
/**
 * An Android `ViewModel`-friendly wrapper for [com.squareup.workflow1.renderWorkflowIn], for use
 * with a [workflow] that requires one input value ([prop]) to run.
 *
 *    ```
 ...
 *   take advantage of some runtime optimizations. E.G., see
 *   [RuntimeConfigOptions.DRAIN_EXCLUSIVE_ACTIONS].
 * @param prop Specifies the sole [PropsT] value to use to render the root workflow. To allow
 *   updates, use the [renderWorkflowIn] overload with a `props: `[StateFlow]`<PropsT>` argument
 *   instead of this one.
 * @param savedStateHandle Used to restore workflow state in a new process. Typically this is the
 *   `savedState: SavedStateHandle` constructor parameter of an androidx
 *   [ViewModel][androidx.lifecycle.ViewModel].
...
/**
 * An Android `ViewModel`-friendly wrapper for [com.squareup.workflow1.renderWorkflowIn], for use
 * with a [workflow] that requires input ([props]) to run.
 *
 * For example, for a workflow that uses [android.content.Intent] as its `PropsT` type, you could do
 * something like this:
 *    ```
...
/**
 * Removes state added to the `savedStateHandle` argument of the Android-specific overload of
 * [renderWorkflowIn]. For use in obscure cases like swapping between different Workflow runtimes in
 * an app. Most apps will not use this function.
 */
public fun SavedStateHandle.removeWorkflowState() {
  remove<Any>(com.squareup.workflow1.android.KEY)
}

@VisibleForTesting internal const val KEY = "com.squareup.workflow1.ui.renderWorkflowIn-snapshot"
