...
internal class WorkflowNodeTest {
  ...
  @Test fun accepts_event() {
    ...
    val node = WorkflowNode(
      ...
      null,
      context,
      emitAppliedActionToParent = { it.copy(output = "tick:${it.output}") }
    )
  ...
  @Test fun accepts_events_sent_to_stale_renderings() {
    ...
    runTest {
      ...
      assertEquals(listOf("tick:event", "tick:event2"), result.map { it.output })
    }
  }
  ...
  @Test fun actionSink_action_emits_output() {
    ...
    val node = WorkflowNode(
      ...
      snapshot = null,
      baseContext = Unconfined,
      emitAppliedActionToParent = { it.copy(output = "output:${it.output}") }
    )
    ...
    runTest {
      ...
      assertEquals("output:hello", result.output)
  ...
  @Test fun child_action_emits_output() {
    ...
    runTest {
      ...
      assertEquals("output:child:hello", result.output)
    }
  }
