...
internal class ChainedWorkflowInterceptorTest {
  @Test
  fun chained_returns_Noop_when_list_is_empty() {
  ...
  @Test
  fun chained_returns_single_element_when_list_size_is_1() {
  ...
  @Test
  fun chained_returns_chained_element_when_list_size_is_2() {
    val list = listOf(object : WorkflowInterceptor {}, object : WorkflowInterceptor {})
    val chained = list.chained()
    ...
    val interceptor1 =
      object : WorkflowInterceptor {
        override fun onSessionStarted(workflowScope: CoroutineScope, session: WorkflowSession) {
          ...
          workflowScope.coroutineContext[Job]!!.invokeOnCompletion { events += "cancelled1" }
        }
      }
    val interceptor2 =
      object : WorkflowInterceptor {
        override fun onSessionStarted(workflowScope: CoroutineScope, session: WorkflowSession) {
          events += "started2"
          workflowScope.coroutineContext[Job]!!.invokeOnCompletion { events += "cancelled2" }
        }
      }
    val chained = listOf(interceptor1, interceptor2).chained()

    launch { chained.onSessionStarted(this, TestSession) }
    advanceUntilIdle()
  ...
  @Test
  fun chains_calls_to_onInitialState_in_left_to_right_order() {
    val interceptor1 =
      object : WorkflowInterceptor {
    ...
    fun initialState(props: String, snapshot: Snapshot?, workflowScope: CoroutineScope): String =
      "($props|${snapshot.readUtf8()})"
  ...
  @Test
  fun chains_calls_to_onPropsChanged_in_left_to_right_order() {
    val interceptor1 =
      object : WorkflowInterceptor {
        override fun <P, S> onPropsChanged(
          ...
          proceed: (P, P, S) -> S,
          session: WorkflowSession,
        ): S = ("s1: " + proceed("old1: $old" as P, "new1: $new" as P, "state1: $state" as S)) as S
      }
    val interceptor2 =
      object : WorkflowInterceptor {
        override fun <P, S> onPropsChanged(
          ...
          proceed: (P, P, S) -> S,
          session: WorkflowSession,
        ): S = ("s2: " + proceed("old2: $old" as P, "new2: $new" as P, "state2: $state" as S)) as S
      }
    val chained = listOf(interceptor1, interceptor2).chained()
    fun onPropsChanged(old: String, new: String, state: String): String = "($old|$new|$state)"
  ...
  @Test
  fun chains_calls_to_onRender_in_left_to_right_order() {
    val interceptor1 =
      object : WorkflowInterceptor {
    ...
    assertEquals("r1: r2: (props2: props1: props|state2: state1: state)", finalRendering)
  }

  @Test
  fun chains_calls_with_RenderContextInterceptor_in_left_to_right_order() {
  ...
  @Test
  fun chains_calls_to_onSnapshotState_in_left_to_right_order() {
    val interceptor1 =
      object : WorkflowInterceptor {
    ...
    val finalSnapshot = chained.onSnapshotState("state", ::snapshotState, TestSession).readUtf8()
