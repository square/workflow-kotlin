...
internal class RememberComposableTest {
  ...
  @Test
  fun rememberSkippableAndRestartable_runs_producer_on_first_compose() = runTest {
    val test = TestComposition(backgroundScope)
    try {
      var calls = 0
      val producer: @Composable () -> Int = {
        calls++
        42
      }
  ...
  @Test
  fun rememberSkippableAndRestartable_skips_producer_when_keys_unchanged() = runTest {
    val test = TestComposition(backgroundScope)
    try {
      val trigger = mutableStateOf(0)
      var calls = 0
      val producer: @Composable () -> String = {
        calls++
        "rendering"
      }
  ...
  @Test
  fun rememberSkippableAndRestartable_runs_producer_when_key1_changes() = runTest {
    val test = TestComposition(backgroundScope)
    try {
      var calls = 0
      val producer: @Composable () -> Int = {
  ...
  @Test
  fun rememberSkippableAndRestartable_runs_producer_when_key2_changes() = runTest {
    val test = TestComposition(backgroundScope)
    try {
      var calls = 0
      val producer: @Composable () -> Int = {
  ...
  @Test
  fun rememberSkippableAndRestartable_caches_with_identity_not_equals() = runTest {
    // Key invariant: workflow renderings are allowed to have throwing equals/hashCode (this is
    // tested at the runtime level by
    // `exceptions_from_renderings_equals_methods_do_not_fail_runtime`).
  ...
  @Test
  fun rememberSkippableComposable_runs_producer_on_first_compose() = runTest {
    val test = TestComposition(backgroundScope)
    try {
      var calls = 0
      val producer: @Composable () -> Int = {
  ...
  @Test
  fun rememberSkippableComposable_runs_producer_when_keys_change() = runTest {
    val test = TestComposition(backgroundScope)
    try {
      var calls = 0
      val producer: @Composable () -> Int = {
  ...
  @Test
  fun rememberSkippableComposable_skips_producer_when_keys_unchanged() = runTest {
    val test = TestComposition(backgroundScope)
    try {
      val trigger = mutableStateOf(0)
      var calls = 0
