...
internal class TrapdoorTest {
  ...
  fun open_block_form_passes_a_trapdoor_into_block() = runTest {
    ...
    try {
      ...
      test.recompose { Trapdoor.open { door -> captured = door } }
  ...
  @Test
  fun open_function_form_returns_a_trapdoor() = runTest {
    val test = TestComposition(backgroundScope)
    try {
  ...
  @Test
  fun runIfValueChanged_does_not_call_on_first_compose() = runTest {
    val test = TestComposition(backgroundScope)
    try {
      val state = mutableStateOf("a")
      val seen = mutableListOf<String>()
      test.recompose { runIfValueChanged(state.value) { old -> seen += old } }
  ...
  @Test
  fun runIfValueChanged_calls_with_previous_value_when_value_changes() = runTest {
    val test = TestComposition(backgroundScope)
    try {
      val state = mutableStateOf("a")
      val seen = mutableListOf<String>()
  ...
  @Test
  fun runIfValueChanged_does_not_call_when_value_unchanged_across_recomposition() = runTest {
    val test = TestComposition(backgroundScope)
    try {
