...
class WorkStealingDispatcherTest {
  ...
  @Test
  fun wrapDispatcherFrom_worksWhenEmpty() = runTest {
    ...
    val job = launch(dispatcher) { wasDispatched = true }
  ...
  @Test
  fun wrapDispatcherFrom_worksWhenInterceptorNotDispatcher() = runTest {
    val dispatcher = WorkStealingDispatcher.wrapDispatcherFrom(NoopContinuationInterceptor())

    expect(0)
    launch(dispatcher) { expect(1) }
    expect(2)
  }

  @Test
  fun wrapDispatcherFrom_takesDispatcherFromContext() = runTest {
    val dispatcher = WorkStealingDispatcher.wrapDispatcherFrom(currentCoroutineContext())

    expect(0)
    launch(dispatcher) { expect(2) }
    expect(1)
  ...
  @Test
  fun wrapDispatcherFrom_wrapsAnotherWorkStealingDispatcher() {
  ...
  @Test
  fun dispatch_runsImmediatelyWhenDelegateIsUnconfined() {
    val dispatcher = WorkStealingDispatcher(Dispatchers.Unconfined)

    expect(0)
    dispatcher.dispatch { expect(1) }
    expect(2)
  }

  @Test
  fun dispatchNested_enqueuesWhenDelegateIsUnconfined() {
    val dispatcher = WorkStealingDispatcher(Dispatchers.Unconfined)
  ...
  @Test
  fun dispatch_queues_whenDelegateNeedsDispatch() {
    val testDispatcher = StandardTestDispatcher()
    val dispatcher = WorkStealingDispatcher(testDispatcher)

    expect(0)
    dispatcher.dispatch { expect(2) }
    expect(1)
  ...
  @Test
  fun dispatch_runsMultipleTasksInOrder_whenDelegateNeedsDispatch() {
    val testDispatcher = StandardTestDispatcher()
    val dispatcher = WorkStealingDispatcher(testDispatcher)
  ...
  @Test
  fun dispatchNested_runsInOrder_whenDelegateNeedsDispatch() {
    val testDispatcher = StandardTestDispatcher()
    val dispatcher = WorkStealingDispatcher(testDispatcher)
    ...
    dispatcher.dispatch {
      expect(2)
      dispatcher.dispatch { expect(5) }

      expect(3)

      dispatcher.dispatch { expect(6) }
      expect(4)
    }
    expect(1)
  ...
  @Test
  fun nestedDispatcher_outerAdvanceAdvancesInner() {
    val base = StandardTestDispatcher()
    val outerDispatcher = WorkStealingDispatcher(base)
    val innerDispatcher = WorkStealingDispatcher(outerDispatcher)

    expect(0)
    outerDispatcher.dispatch { expect(2) }
    innerDispatcher.dispatch { expect(3) }
    expect(1)
  ...
  @Test
  fun nestedDispatcher_innerDoesNotAdvanceAdvanceOuter() {
    val base = StandardTestDispatcher()
    val outerDispatcher = WorkStealingDispatcher(base)
    val innerDispatcher = WorkStealingDispatcher(outerDispatcher)

    expect(0)
    outerDispatcher.dispatch { expect(4) }
    innerDispatcher.dispatch { expect(2) }
    expect(1)
  ...
  @Test
  fun dispatch_interceptsAndResumesContinuation() {
    val baseDispatcher =
  ...
  @Test
  fun dispatch_interceptsAndReleasesContinuationWhenIntercepted() {
  ...
  @Test
  fun dispatch_interceptsAndReleasesContinuationWhenAdvanced() {
    var intercepted: Continuation<*>? = null
  ...
  @Test
  fun dispatch_doesNotReleaseContinuationWhenNotIntercepted() {
    val baseDispatcher =
      object : ContinuationInterceptor, AbstractCoroutineContextElement(ContinuationInterceptor) {
        override fun <T> interceptContinuation(continuation: Continuation<T>): Continuation<T> {
          expect(1)
  ...
  @Test
  fun advanceUntilIdle_drainsQueueWhileWaitingForDispatch() {
    val testDispatcher = StandardTestDispatcher()
    val dispatcher = WorkStealingDispatcher(testDispatcher)
  ...
  @Test
  fun advanceUntilIdle_handlesNestedDispatches() {
    val testDispatcher = StandardTestDispatcher()
    val dispatcher = WorkStealingDispatcher(testDispatcher)
  ...
  @Test
  fun advanceUntilIdle_canBeCalledReentrantly() {
    val testDispatcher = StandardTestDispatcher()
    val dispatcher = WorkStealingDispatcher(testDispatcher)
   ...
   */
  @Test
  fun advanceUntilIdle_isEager_whenCalledReentrantlyWhileMultipleTasksQueued() {
    val testDispatcher = StandardTestDispatcher()
    val dispatcher = WorkStealingDispatcher(testDispatcher)
  ...
  @Test
  fun advanceUntilIdle_noopsWhenNoTasks() {
    val dispatcher = WorkStealingDispatcher(StandardTestDispatcher())
  ...
  @Test
  fun doesNotDoubleDispatch_whenDispatchedAfterAdvance() {
  ...
  @Test
  fun doesNotDoubleDispatch_whenAdvancedAfterDispatch() {
    val baseDispatcher = RecordingDispatcher()
    val dispatcher = WorkStealingDispatcher(baseDispatcher)
  ...
  fun limitedParallelism_unsupported_whenDelegateNotDispatcher() {
    ...
    assertFailsWith<UnsupportedOperationException> { dispatcher.limitedParallelism(2) }
  }
  ...
  fun limitedParallelism_limitsParallelism() {
    ...
    var tasksRan = 0
    repeat(3) { limited.dispatch { tasksRan++ } }
  ...
  fun limitedParallelism_isAlsoWorkStealing() {
    ...
    expect(0)
    limited.dispatch { expect(2) }
    limited.dispatch { expect(3) }
    limited.dispatch { expect(4) }
    expect(1)
   ...
   */
  @Test
  fun preservesDelegateDispatchOrdering() {
    val baseDispatcher = RecordingDispatcher()
    val dispatcher = WorkStealingDispatcher(baseDispatcher)
    ...
    baseDispatcher.blocks.asReversed().forEach { it.run() }
    expect(5)
  }
  ...
  @Test
  fun integration_unconfined() = runTest {
    val dispatcher = WorkStealingDispatcher(Dispatchers.Unconfined)
  ...
  @Test
  fun integration_confined_whenAdvanced() = runTest {
  ...
  @Test
  fun integration_confined_whenDispatched() = runTest {
    val testDispatcher = StandardTestDispatcher(testScheduler)
    val dispatcher = WorkStealingDispatcher(testDispatcher)
  ...
  @Test
  fun integration_yield_whenAdvanced() = runTest {
    val testDispatcher = StandardTestDispatcher(testScheduler)
    val dispatcher = WorkStealingDispatcher(testDispatcher)
  ...
  @Test
  fun integration_yield_whenDispatched() = runTest {
    val testDispatcher = StandardTestDispatcher(testScheduler)
    val dispatcher = WorkStealingDispatcher(testDispatcher)
  ...
  @Test
  fun integration_delay_whenAdvanced() = runTest {
    val testDispatcher = StandardTestDispatcher(testScheduler)
    val dispatcher = WorkStealingDispatcher(testDispatcher)
  ...
  @Test
  fun integration_delay_whenDispatched() = runTest {
    val testDispatcher = StandardTestDispatcher(testScheduler)
    val dispatcher = WorkStealingDispatcher(testDispatcher)
  ...
  @Test
  fun integration_error_noFinally_whenAdvanced() {
  ...
  @Test
  fun integration_error_noFinally_whenDispatched() {
    val exceptions = mutableListOf<Throwable>()
    val testDispatcher = StandardTestDispatcher()
    val dispatcher = WorkStealingDispatcher(testDispatcher)
    val scope =
  ...
  @Test
  fun integration_error_withCatch_whenAdvanced() {
    val exceptions = mutableListOf<Throwable>()
    val testDispatcher = StandardTestDispatcher()
    val dispatcher = WorkStealingDispatcher(testDispatcher)
    val scope =
      CoroutineScope(
        dispatcher + CoroutineExceptionHandler { _, throwable -> exceptions += throwable }
      )
  ...
  @Test
  fun integration_error_withCatch_whenDispatched() {
    val exceptions = mutableListOf<Throwable>()
    val testDispatcher = StandardTestDispatcher()
    val dispatcher = WorkStealingDispatcher(testDispatcher)
    val scope =
  ...
  @Test
  fun integration_error_withFinally_whenAdvanced() {
    val exceptions = mutableListOf<Throwable>()
    val testDispatcher = StandardTestDispatcher()
    val dispatcher = WorkStealingDispatcher(testDispatcher)
    val scope =
  ...
  @Test
  fun integration_error_withFinally_whenDispatched() {
    val exceptions = mutableListOf<Throwable>()
    val testDispatcher = StandardTestDispatcher()
    val dispatcher = WorkStealingDispatcher(testDispatcher)
    val scope =
   ...
   */
  @Test
  fun integration_yieldInLoop_whenAdvanced() = runTest {
    val testDispatcher = StandardTestDispatcher(testScheduler)
    val dispatcher = WorkStealingDispatcher(testDispatcher)
   ...
   */
  @Test
  fun integration_yieldInLoop_whenDispatched() = runTest {
    val testDispatcher = StandardTestDispatcher(testScheduler)
    val dispatcher = WorkStealingDispatcher(testDispatcher)
  ...
  private class NoopContinuationInterceptor :
    ContinuationInterceptor, AbstractCoroutineContextElement(ContinuationInterceptor) {
