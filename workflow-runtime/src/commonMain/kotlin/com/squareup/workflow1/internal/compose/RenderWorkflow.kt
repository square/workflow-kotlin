...
internal fun <PropsT, OutputT, RenderingT> renderWorkflow(
  ...
  val renderContext = rememberComposeRenderContext(
    workflow = workflow,
    initialProps = props,
    config = config,
...
/**
 * Exposes [renderWorkflowRestartableImpl] as a composable function, to get the compiler to wire
 * up the `changed` flags.
 */
@Suppress("UNCHECKED_CAST")
private val renderWorkflowRestartable = ::renderWorkflowRestartableImpl as @Composable (
  Any?,
  ((Any?) -> Unit)?,
