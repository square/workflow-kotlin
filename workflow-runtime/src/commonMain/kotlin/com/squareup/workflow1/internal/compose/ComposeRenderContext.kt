...
private constructor(
  ...
  override fun <ResultT> remember(
    ...
    requireTrapdoor("remember").inMovableGroup(GROUP_KEY, key) {
      // RememberStore keys the lifetime of the memoization off the result type and inputs in
      // addition
      ...
      // reset the state for this particular remember call, so they don't need to be part of the
      // data
  ...
  @Composable
  private fun updatePropsAndOutput(props: P, onOutput: ((O) -> Unit)?) {
    ...
    Trapdoor.runIfValueChanged(props) { oldProps ->
      workflowTracer.traceNoFinally(OnPropsChanged) {
        @Suppress("UNCHECKED_CAST")
        state.value =
          interceptedWorkflow.onPropsChanged(old = oldProps, new = props, state = state.value)
      }
    }
    SideEffect {
