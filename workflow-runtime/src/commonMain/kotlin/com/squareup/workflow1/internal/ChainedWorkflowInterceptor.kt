...
internal class ChainedWorkflowInterceptor(private val interceptors: List<WorkflowInterceptor>) :
  WorkflowInterceptor {
  ...
  override fun <P, S, O> onSessionCancelled(
    ...
    interceptors.forEach {
      it.onSessionCancelled(cause = cause, droppedActions = droppedActions, session = session)
    }
  }
  ...
  override fun <P, R> onRenderAndSnapshot(
    ...
    val chainedProceed =
      interceptors.foldRight(proceed) { workflowInterceptor, proceedAcc ->
        { renderProps -> workflowInterceptor.onRenderAndSnapshot(renderProps, proceedAcc, session) }
      }
  ...
  override fun <S> onSnapshotState(
    ...
    val chainedProceed =
      interceptors.foldRight(proceed) { workflowInterceptor, proceedAcc ->
        { state -> workflowInterceptor.onSnapshotState(state, proceedAcc, session) }
      }
    return chainedProceed(state)
  }

  override fun onRuntimeUpdate(update: RuntimeUpdate) {
    interceptors.forEach { interceptor -> interceptor.onRuntimeUpdate(update) }
  }
