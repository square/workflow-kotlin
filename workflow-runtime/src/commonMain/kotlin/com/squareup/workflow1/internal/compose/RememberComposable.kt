 ...
 * passed and [producer] hasn't been invalidated itself (i.e. due to a state change), then
 * [producer] is skipped (i.e. state is kept in the composition but it is not recomposed) and the
 * cached value is returned. If a different [producer] instance is passed, it's composed and its
 * return value is cached before being returned.
 *
 * [producer] is _not_ treated as an implicit key: only the initial [producer] instance passed to
 * this function will ever be called. The compose compiler will normally auto-remember lambdas
 * passed to composables, but _only if they return `Unit`_. Since [producer] has a non-`Unit` return
 * type, it will never be auto-remembered. So this function always remembers [producer] itself. If
 * we treated [producer] as a key, it would always be recomposed unless the caller explicitly
 * remembered it. To update the producer logic, store it in a [MutableState]:
 *
...
internal fun <R> rememberSkippableComposable(
  ...
  rememberSkippableComposableImpl(
    arg1 = key1,
    arg2 = key2,
    arg3 = Unit,
    producer = producer,
...
// // We don't need the compiler to generate groups because the impl non-Composable function
// explicitly
// // creates its own groups anyway.
// @ExplicitGroupsComposable
// @Composable
...
//   // Inner group is necessary to be able to skip calling producer. We need a nested group because
// we
...
//
//   // We don't consider producer in this change detection code since we don't want to treat it as
// a
//   // key.
//   var dirty = changed
...
//
//   // region Update cache
//   // Cache the return value in case we skipped above. Composer APIs require always reading the
// value
 ...
 * same composition phase of the same frame. When this happens, the same caching behavior applies:
 * if the same instance of [producer] is passed in then it will be skipped and the cached value will
 * be returned.
 *
 * [producer] is _not_ treated as an implicit key: only the initial [producer] instance passed to
 * this function will ever be called. The compose compiler will normally auto-remember lambdas
...
@PublishedApi
internal fun <R> rSKC_after(newValue: Any?, composer: Composer): R {
  composer.endReplaceGroup()
