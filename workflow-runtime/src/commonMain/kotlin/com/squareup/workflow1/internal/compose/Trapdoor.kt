...
internal value class Trapdoor(private val composer: Composer) {
  ...
  fun <R> inMovableGroup(key: Int, dataKey: Any?, content: @Composable () -> R): R {
    @Suppress("WRONG_INVOCATION_KIND") contract { callsInPlace(content, kind = EXACTLY_ONCE) }
    ...
    return invokableContent.invoke(composer, 0).also { composer.endMovableGroup() }
  }
