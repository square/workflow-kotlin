...
/**
 * Interface for a pluggable [WorkflowRuntimeTracer] that can be used with the monitoring provided
 * by [WorkflowRuntimeMonitor]. Note that this extends [WorkflowInterceptor] to allow
 * [WorkflowRuntimeTracer]s to use those methods as well for tracing, except in a couple of cases
 * where it provides enhanced hooks, it prevents the underlying hooks from being used.
 */
public abstract class WorkflowRuntimeTracer : WorkflowInterceptor {
   ...
   *
   * This should not really be overridden (only [ChainedWorkflowRuntimeTracer] uses that to chain
   * this for all [WorkflowRuntimeTracer]).
   */
  public open fun attachRuntimeContext(workflowRuntimeTraceContext: RuntimeTraceContext) {
  ...
  protected val WorkflowSession.name: String
    inline get() =
      requireNotNull(sessionInfo[sessionId]) {
  ...
  public open fun onRootPropsChanged(session: WorkflowSession) = Unit
  ...
  public open fun onWorkflowSessionStopped(sessionId: Long) = Unit

  /**
   * A [RuntimeUpdate] has occurred, we may want to trace something. This is to be preferred to the
   * [WorkflowInterceptor.onRuntimeUpdate] method as it provides more context about what is
   ...
   */
  final override fun onSessionStarted(workflowScope: CoroutineScope, session: WorkflowSession) {
