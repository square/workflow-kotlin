...
/**
 * PLEASE NOTE: these log lines are turned into strings in production, all the time, and there are
 * many of them. So we take good care of keeping this code tight and performant, by leveraging
 * StringBuilder and not doing any Kotlin string interpolation (which creates intermediate builder
 * objects) as well as preferring the Java builder methods over the kotlin extension functions (e.g.
 * append('\n') instead of appendLine()): even though the Kotlin extension functions are inlined,
 * they also strengthen the types from the Java compatibility "maybe nullable" to "definitely not
 * null" and the compiler adds 2 call to Intrinsics.checkNotNullExpressionValue per use (for the
 * callee and the result, both of which are StringBuilder!).
 */
...
private data class LogLineLimits(val maxLogLineLength: Int, val crashOnLogLineOverflow: Boolean)
...
private fun StringBuilder.enforceLogLineLimit(
  ...
  wfEllipsizeEndInPlacePreservingFinalNewline(startIndex = lineStart, maxLength = maxLogLineLength)
}
...
/**
 * The "UI" has updated, whatever that means for your app. You must manually add this to the
 * [WorkflowRuntimeMonitor]'s [RuntimeUpdates] by calling [WorkflowRuntimeMonitor.addRuntimeUpdate].
 */
public class UiUpdateLogLine
private constructor(val note: String, private val logLineLimits: LogLineLimits?) :
  RuntimeUpdateLogLine {
  public constructor(note: String) : this(note = note, logLineLimits = null)
  ...
  override fun log(builder: StringBuilder) {
    logWithLimits(builder, logLineLimits) {
      builder.append("UI UPDATE: ").append(note).append('\n')
    }
  }
}
...
public data object SkipLogLine : RuntimeUpdateLogLine {
  override fun log(builder: StringBuilder) {
    builder.append("SKIP RENDER").append('\n')
  }
}

/** An action that was queued but got dropped because the Workflow session ended. */
public class ActionDroppedLogLine
private constructor(val actionName: String, private val logLineLimits: LogLineLimits?) :
  RuntimeUpdateLogLine {
  public constructor(actionName: String) : this(actionName = actionName, logLineLimits = null)

  internal fun copyWithLogLimits(
    maxLogLineLength: Int,
    crashOnLogLineOverflow: Boolean,
  ): ActionDroppedLogLine =
    ActionDroppedLogLine(
  ...
  override fun log(builder: StringBuilder) {
    logWithLimits(builder, logLineLimits) {
      builder.append("DROPPED: ").append(actionName).append('\n')
    }
  }
}
  ...
  override fun log(builder: StringBuilder) {
    logWithLimits(builder, logLineLimits) {
      ...
      if (nextActionName != null || nextWorkflowName != null) {
        ...
        nextActionName?.let { builder.append("A(").append(it).append(")") }
        nextWorkflowName?.let { builder.append("/W(").append(it).append(")") }
      }
      builder.append('\n')
