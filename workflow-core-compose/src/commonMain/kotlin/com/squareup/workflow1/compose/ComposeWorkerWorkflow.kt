...
import com.squareup.workflow1.collectToSink
...
internal class ComposeWorkerWorkflow<OutputT>(
  val workerType: KType,
  private val key: String
) : StatefulComposeWorkflow<Worker<OutputT>, Int, OutputT, Unit>(),
  ImpostorWorkflow {
  ...
  override fun render(
    ...
    renderState: Int,
    context: StatefulWorkflow<Worker<OutputT>, Int, OutputT, Unit>.RenderContext
  ) {
  ...
  override fun Rendering(
    ...
    val stateKey = remember(renderState) { renderState.toString() }
    SideEffect {
      context.runningSideEffect(stateKey) {
        runWorker(renderProps, key, context.actionSink)
      }
    }
  }
